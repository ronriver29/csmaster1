      ...
      <ol class="text-justify" type="1">
        ...
        <li>What strategies the cooperative shall implement to ensure the support of the members?<br>
         ...
         <!--  <p class="text-justify font-weight-normal"><span style="font-family: DejaVu Sans; sans-serif;"><?php echo ($survey_info->strategies_support_members[1] == "1") ? "X - ": ""?></span>Commitment on lending policies</p> -->
          ...
          <?php
          $strategs = '';
            if($survey_info->strategies_support_members[2]!=$survey_info_orig->strategies_support_members[2])
            {
              $strategs = '<b><span style="font-family: DejaVu Sans; sans-serif;">'. ($survey_info->strategies_support_members[2] == "1" ? "X - ": "").'</span>Active participation in cooperative affairs</b>';
            }
            else
            {
              $strategs = '<span style="font-family: DejaVu Sans; sans-serif;">'. ($survey_info->strategies_support_members[2] == "1" ? "X - ": "").'</span>Active participation in cooperative affairs';
            }
          ?>
        ...
        $trans_business_with2='';
          $trans_business_with1 = ($survey_info->transact_business_with == "1" ? "X - a) members only": "a) members only");
          $trans_business_with_orig1 = ($survey_info_orig->transact_business_with == "1" ? "X - a) members only": "a) members only");

            $trans_business_with2 = ($survey_info->transact_business_with == "2" ? "X - b) members and non-members": "b) members and non-members");
         

           $trans_business_with_orig2 = ($survey_info_orig->transact_business_with == "2" ? "X - b) members and non-members": "b) members and non-members");
           echo strlen($trans_business_with_orig1). $trans_business_with_orig1.' '.strlen($trans_business_with1).' '.$trans_business_with1;
           if($trans_business_with1 !=  $trans_business_with_orig1)
           {
              $trans_business_with1 = '<b>'.$trans_business_with1.'</b>';
           }
            if($trans_business_with2 !=  $trans_business_with_orig2)
           {
        ...
        <li>Are you intending to transact business with
          <p class="text-justify "><span style="font-family: DejaVu Sans; sans-serif;"><?= $trans_business_with1?></span>  <span style="font-family: DejaVu Sans; sans-serif;"><?=  $trans_business_with2?></span>  </p>
        </li>
