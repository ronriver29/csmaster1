...
class Article_update_model extends CI_Model{
  ...
  public function update_article_primary($article_coop_id,$article_info){
    ...
    $this->db->trans_begin();
    $query_check = $this->db->query("select * from articles_of_cooperation where cooperatives_id = $article_coop_id");
    ...
    {
      $article_info['cooperatives_id'] = $article_coop_id;
      $this->db->insert('articles_of_cooperation',$article_info);
        if($this->db->trans_status() === FALSE){
          $this->db->trans_rollback();
          return false;
        }else{
  ...
  public function check_article_primary_complete($article_coop_id){
    ...
    if($query2->num_rows()>0)
    {
       $data2 = $query2->row();
        ...
        if(isset($data->kinds_of_members)){
