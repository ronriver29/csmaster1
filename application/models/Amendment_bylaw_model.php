...
class Amendment_bylaw_model extends CI_Model{
  ...
  public function get_bylaw_by_coop_id($coop_id){
    ...
    $query = $this->db->get_where('amendment_bylaws',array('cooperatives_id'=>$data));
    return $query->row();
  }
  public function update_bylaw_primary($bylaw_coop_id,$bylaw_info){
    ...
    $get_record = $this->db->where("cooperatives_id",$bylaw_coop_id)->get("amendment_bylaws");
    if($get_record->num_rows()==0) {
        $this->db->insert('amendment_bylaws', array('cooperatives_id'=>$bylaw_coop_id));
  ...
  public function check_bylaw_primary_complete($bylaw_coop_id){
    $data = array();
    $counter = 0;
    $query = $this->db->get_where('amendment_bylaws',array('cooperatives_id'=>$bylaw_coop_id));
    if($query->num_rows()>0) {
        $data = $query->row();
    }
    if(isset($data->id)) {
  ...
  public function check_minimum_associate_pay($ajax){
    $decoded_id = $this->encryption->decrypt(decrypt_custom($ajax['coop_id']));
    $this->db->select('amendment_bylaws.associate_percentage_shares_pay');
    $this->db->from('amendment_bylaws');
    $this->db->join('cooperatives','cooperatives.id = amendment_bylaws.cooperatives_id','inner');
    $this->db->where(array('cooperatives_id'=>$decoded_id));
    $query = $this->db->get();
