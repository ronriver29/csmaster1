...
class Economic_survey_update_model extends CI_Model{
  ...
  public function update_economic_survey($survey_coop_id,$survey_info){
    ...
    if($this->check_if_had_ec_survey($survey_coop_id))
    {
      $this->db->trans_begin();
      ...
      $survey_info['cooperatives_id'] = $survey_coop_id;
      $this->db->trans_begin();
      $this->db->insert('economic_survey',$survey_info);
      if($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
        return false;
      }else{
...
//  } 
   ...
   public function check_if_had_ec_survey($coop_id)
  {
    $query = $this->db->get_where('economic_survey',array('cooperatives_id'=>$coop_id));
    if($query->num_rows() == 1)
    {
      return true;
    }
