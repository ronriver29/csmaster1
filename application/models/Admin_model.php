...
class Admin_model extends CI_Model{
  ...
  public function get_all_user2($first_name,$middle_name,$last_name,$regno,$email,$contact_number,$limit){
    ...
    $this->db->join('registeredcoop rc','c.id = rc.application_id','LEFT');
    $this->db->where('u.is_verified = 1 AND (u.regno IS NULL OR u.regno = "")'.$and_where);
    $query = $this->db->get();
    return $query->result_array();
  }

  public function get_all_new_user2($first_name,$middle_name,$last_name,$email,$regno,$contact_number,$limit){
    $where_array = array();
       if($first_name != '') {
           $where_array[] = "u.first_name LIKE '%".$first_name."%'";
       }
...
public function add_admin_director($data,$raw_pass){
    ...
    {
      $chk_qry = $this->db->get_where('admin',array('region_code'=>$data['region_code'],'access_level'=>3,'access_name'=>'Director','active'=>1));
      if($chk_qry->num_rows()>0)
      {
