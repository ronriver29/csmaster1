...
class Bylaw_update_model extends CI_Model{
  ...
  public function update_bylaw_primary($bylaw_coop_id,$bylaw_info){
    ...
    if($this->check_if_had_bylaws($bylaw_coop_id))
    {
      $this->db->trans_begin();
    ...
    {
      $bylaw_info['cooperatives_id'] = $bylaw_coop_id;
      $this->db->trans_begin();
      $this->db->insert('bylaws',$bylaw_info);
      if($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
        return false;
      }else{
  ...
  public function check_if_had_bylaws($coop_id)
  {
    $query = $this->db->get_where('bylaws',array('cooperatives_id'=>$coop_id));
    if($query->num_rows() == 1)
    {
      return true;
    }
