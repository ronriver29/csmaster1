...
public $regNo='';
  public function __construct()
  {
    ...
    public function count_applied_coop($amendment_fed_id)
    {   
        $this->amendment_fed_id= $this->security->xss_clean($amendment_fed_id);
        $this->db->select("id");
    ...
    public function applied_coop_amendment($amendment_id){
     ...
     public function list_members($amendment_id){
    ...
    public function get_applied_coop_admin($amendment_id,$limit,$start,$coopName,$regNo){
     ...
     public function count_applied_coop_admin($amendment_fed_id,$coopName,$regNo)
    {   
        $this->coopName = (strlen($coopName)>0 ? " AND amendment_affiliators.coopName like '$coopName%'" : "");
        $this->regNo = (strlen($regNo)>0 ? " AND amendment_affiliators.regNo like '$regNo%'" : "");
        $this->amendment_fed_id= $this->security->xss_clean($amendment_fed_id);
    ...
    public function get_all_affiliators_of_coop($amendment_fed_id){
    $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
...
public function get_registered_coop_tech_service($area_of_operation,$addresscode,$type_of_cooperative){
    ...
    public function get_all_cooperator_of_coop($amendment_fed_id){
        $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
        $this->db->select('amendment_affiliators.*');
        $this->db->from('amendment_affiliators');
        $this->db->where('amendment_fed_id', $amendment_fed_id);
...
//        $query = $this->db->get();
    ...
    public function is_requirements_complete($amendment_fed_id){
       $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    ...
    public function check_no_of_directors($amendment_fed_id){
    ...
    public function check_directors_odd_number($amendment_fed_id){
         $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
        $position = array('Chairperson', 'Vice-Chairperson', 'Board of Director');
        $this->db->where('amendment_fed_id',$this->amendment_fed_id);
        $this->db->where('(position LIKE "Chairperson%" OR position LIKE "Vice-Chairperson%" OR position LIKE "Board of Director%")');
    ...
    public function check_chairperson($amendment_fed_id){
    ...
    public function check_vicechairperson($amendment_fed_id){
    ...
    public function check_treasurer($amendment_fed_id){
    ...
    public function check_secretary($amendment_fed_id){
    ...
    public function check_all_minimum_regular_subscription($amendment_fed_id){
        $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
       
    //    $temp = $this->bylaw_model->get_bylaw_by_coop_id($amendment_fed_id)->regular_percentage_shares_subscription;
    ...
    public function check_all_minimum_regular_pay($amendment_fed_id){
        $this->amendment_fed_id= $this->security->xss_clean($amendment_fed_id);
        // $user_id = $this->security->xss_clean($user_id);
    //    $temp = $this->bylaw_model->get_bylaw_by_coop_id($amendment_fed_id)->regular_percentage_shares_pay;
  ...
  public function get_all_amendment_affiliators_of_coop($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->select('amendment_affiliators.*');
    $this->db->from('amendment_affiliators');
    $this->db->where('user_id', $amendment_fed_id);
    $query=$this->db->get();
$this->last_query = $this->db->last_query();
    $data = $query->result_array();
    return $data;
  ...
  public function get_affiliator_info($amendment_fed_id){
    $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->select('amendment_affiliators.*');
    $this->db->from('amendment_affiliators');
    $this->db->where('amendment_fed_id',  $this->amendment_fed_id);
  ...
  public function get_total_regular($amendment_fed_id){
    $this->amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
  ...
  public function get_list_of_directors($amendment_fed_id){
    $position = array('Chairperson', 'Vice-Chairperson', 'Board of Director');
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->where('amendment_fed_id',$amendment_fed_id);
    $this->db->where('(position LIKE "Chairperson%" OR position LIKE "Vice-Chairperson%" OR position LIKE "Board of Director%")');
  ...
  public function no_of_directors($amendment_fed_id){
    $position = array('Chairperson', 'Vice-Chairperson', 'Board of Director');
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->where('amendment_fed_id',$amendment_fed_id);
    $this->db->where('(position LIKE "Chairperson%" OR position LIKE "Vice-Chairperson%" OR position LIKE "Board of Director%")');
  ...
  public function get_all_regular_cooperator_of_coop($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->order_by('representative','asc');
    $query = $this->db->get_where('amendment_affiliators',array('user_id' => $amendment_fed_id));
    $data = $query->result_array();
    return $data;
  }

  public function get_treasurer_of_coop($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $query = $this->db->get_where('amendment_affiliators','amendment_fed_id = '.$amendment_fed_id.' AND position  like "%Treasurer%"');
    // $this->db->from('amendment_affiliators');
    $data = $query->row();
    unset($query);
    unset($amendment_fed_id);
  ...
  public function get_all_cooperator_of_coop_regular($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    ...
    $this->db->join('refprovince', 'refprovince.provCode = refcitymun.provCode','left');
    $this->db->join('refregion', 'refregion.regCode = refprovince.regCode','left');
    $this->db->where('type_of_member = "Regular" AND amendment_fed_id = '.$amendment_fed_id.'');
  ...
  public function is_position_available($user_id,$position){
    ...
    $this->db->where('amendment_fed_id',$decoded_id);
    $this->db->where('('.$or.')');
  ...
  public function get_total_number_of_cooperators($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $this->db->where('user_id',$amendment_fed_id);
    $this->db->from('amendment_affiliators');
    return $this->db->count_all_results();
  ...
  public function get_all_board_of_director_only($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $query = $this->db->get_where('amendment_affiliators','amendment_fed_id = '.$amendment_fed_id.' AND (position LIKE "Board of Director%" OR position LIKE "%Board of Director")');
    $data = $query->result_array();
    return  $data;
  }

  public function get_chairperson_of_coop($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $query = $this->db->get_where('amendment_affiliators','amendment_fed_id = '.$amendment_fed_id.' AND (position LIKE "Chairperson%" OR position LIKE "%Chairperson")');
    $data = $query->row();
    return $data;
  }

  public function get_vicechairperson_of_coop($amendment_fed_id){
    $amendment_fed_id = $this->security->xss_clean($amendment_fed_id);
    $query = $this->db->get_where('amendment_affiliators','amendment_fed_id = '.$amendment_fed_id.' AND (position LIKE "Vice-Chairperson%" OR position LIKE "%Vice-Chairperson")');
    // 'user_id = '.$amendment_fed_id.' AND (position LIKE "Treasurer%" OR position LIKE "%Treasurer")'
    $data = $query->row();
    return $data;
  }
}
