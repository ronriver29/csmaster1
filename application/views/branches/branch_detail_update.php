<div class="row mb-2">
  ...
  <?php if(!$is_client){?>
    <a class="btn btn-secondary btn-sm btn-block"  href="<?php echo base_url();?>updated_branch_info_registered" role="button"><i class="fas fa-arrow-left"></i> Go Back</a>
  <?php } else {?>
...
<div class="row mt-3">
    <div class="col-sm-12 col-md-12">
      <div class="alert alert-info" role="alert">
        <p><center>Update the Basic Information to update the steps.</center></p>
      </div>
    </div>
  </div>
<div class="row mb-2">
  <div class="col-sm-12 col-md-4">
    <div class="card border-top-blue shadow-sm mb-4">
      ...
      <div class="card-body">
        ...
        <p class="text-muted">
          <?php if($branch_info->house_blk_no==null && $branch_info->street==null) $x=''; else $x=', ';
            if(isset($branch_info->city)){
              $branch_info->city = $branch_info->city;
            } else {
              $branch_info->city = '';
            }

            if(isset($branch_info->brgy)){
              $branch_info->brgy = $branch_info->brgy;
            } else {
              $branch_info->brgy = '';
            }

            if(isset($branch_info->province)){
              $branch_info->province = $branch_info->province;
            } else {
              $branch_info->province = '';
            }

            if(isset($branch_info->region)){
              $branch_info->region = $branch_info->region;
            } else {
              $branch_info->region = '';
            }
          ?>
      ...
      <?php if(($is_client && $branch_info->status==21 || $branch_info->status==0) || (!$is_client)): ?>
        <div class="card-footer">
          <a href="<?php echo base_url();?>branch_update/<?= $encrypted_id ?>/bupdate" class="btn btn-block btn-color-blue"><i class='fas fa-edit'></i> Update Basic Information</a>
        </div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="col-sm-12 col-md-8">
    <ul class="list-group">
      <li class="list-group-item  flex-column align-items-start">
        ...
        <?php if($branch_info->status!= 0): ?>
          <small class="text-muted">
            <a href="<?php echo base_url();?>branch_update/<?= $encrypted_id ?>/documents_branch_update" class="btn btn-info btn-sm">View</a>
          </small>
        <?php endif; ?>
      </li>
        <li class="list-group-item  flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between">
            ...
            <small class="text-muted">
              ...
              <?php if(($branch_info->addrCode != '' && $branch_info->status == 80)): ?>
                <?php if(substr($branch_info->addrCode,0,2) == substr($subcode,0,2) || $admin_info->region_code == "00"){ ?>
                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#approveBranchModal"  data-cname="<?= $branch_info->coopName?>" data-coopid="<?= encrypt_custom($this->encryption->encrypt($branch_info->id))?>" >Submit All</button>
              <?php } ?>
              <?php endif; ?>
              </div>
            </small>
      ...
      <ul class="list-group">
        ...
        <li class="list-group-item  flex-column align-items-start">
          ...
          <li class="list-group-item  flex-column align-items-start">
            ...
            <?php if($branch_info->addrCode != '' && $branch_info->status != 80 && $branch_info->status != 81): ?>
            <small class="text-muted">
              <a href="<?php echo base_url();?>branch_update/<?= $encrypted_id ?>/evaluate" class="btn btn-color-blue btnFinalize btn-sm ">Submit</a>
            </small>
            <?php endif; ?>
          </li>
          <li class="list-group-item  flex-column align-items-start">
