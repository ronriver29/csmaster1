    ...
    <div class="card border-top-blue shadow-sm mb-4">
      ...
      <div class="card-body">
        ...
        <?php if($is_client) : ?>
          ...
          <p class="text-muted">
            ...
            <?php if($branch_info->status==40) echo "TRANSFER"; ?>
            <?php if($branch_info->status==41) echo "LETTER SUBMISSION SUCCESSFUL"; ?>
            <?php if($branch_info->status==42) echo "LETTER SUBMISSION SUCCESSFUL"; ?>
            <?php if($branch_info->status==43 && $branch_info->sent_lapse_notif == 0) echo "LETTER RECEIVED"; ?>
            <?php if($branch_info->status==43 && $branch_info->sent_lapse_notif == 1) echo "FOR UPLOADING"; ?>
            <?php if($branch_info->status==44) echo "FOR VALIDATION"; ?>
            <?php if($branch_info->status==45) echo "FOR VALIDATION"; ?>
            <?php if($branch_info->status==46) echo "FOR EVALUATION"; ?>
            <?php if($branch_info->status==47) echo "APPLICATION FOR TRANSFER-SUBMITTED BY SR.CDS / FOR EVALUATION"; ?>
            <?php if($branch_info->status==48) echo "APPROVED FOR PRINTING AND SUBMISSION OF THE REQUIREMENTS"; ?>
            <?php if($branch_info->status==49) echo "DEFERRED"; ?>
            <?php if($branch_info->status==50) echo "DENIED"; ?>
            <?php if($branch_info->status==51) echo "FOR PAYMENT"; ?>
            <?php if($branch_info->status==52) echo "SAVE O.R"; ?>
            <?php if($branch_info->status==53) echo "FOR PRINTING OF ORDER OF TRANSFER"; ?>
            <?php if($branch_info->status==54) echo "ISSUED ORDER OF TRANSFER"; ?>
      ...
      <?php if(($is_client && ($branch_info->status==40))): ?>
        <div class="card-footer">
          <a href="<?php echo base_url();?>for_closure/<?= $encrypted_id ?>/bupdate" class="btn btn-block btn-color-blue"><i class='fas fa-edit'></i> Update Basic Information</a>
        </div>
      <?php endif; ?>
    </div>
  </div>
    ...
    <div class="col-sm-12 col-md-8">
      <ul class="list-group">
        <li class="list-group-item  flex-column align-items-start">
          ...
          <a href="<?php echo base_url();?>for_closure/<?= $encrypted_id ?>/bupdate" class="btn btn-info btn-sm">View</a>
        </li>
        <li class="list-group-item  flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between">
          ...
          <p class="mb-1 font-italic">Submission of Letter of Intent
          </p>
          <?php if($branch_info->status!= 0): ?>
            <small class="text-muted">
              <a href="<?php echo base_url();?>branches/<?= $encrypted_id ?>/documents_transfer" class="btn btn-info btn-sm">View</a>
            </small>
          <?php endif;?>
        </li>
          <li class="list-group-item  flex-column align-items-start">
            ...
            <?php if(($branch_info->status == 21 || $branch_info->status == 40) && $document_43): ?>
            <?php if($branch_info->reason != ''): ?>
              <small class="text-muted">
                <a href="<?php echo base_url();?>bns_transfer/<?= $encrypted_id ?>/evaluate" class="btn btn-color-blue btnFinalize btn-sm ">Submit</a>
              </small>
            <?php else : ?>
                <i style="color:red;">* Update reason on Basic Information to submit</i>
              <?php endif; ?>
            <?php endif; ?>
          </li>
          <li class="list-group-item  flex-column align-items-start">
            <div class="d-flex w-100 justify-content-between">
              ...
              <small class="text-muted">
                <?php if($document_47 && $document_48 && $document_49) :?>
                <span class="badge badge-success">COMPLETE</span>
                <?php endif; ?>
                <?php if(!$document_47 && !$document_48 && !$document_49) :?>
                <span class="badge badge-secondary">PENDING</span>
            ...
            <?php if($branch_info->status == 33 || $branch_info->status == 35 || $branch_info->status == 43 || $branch_info->status == 48 || $branch_info->status == 49 || $branch_info->status >= 51): ?>
              <small class="text-muted">
                <?php
                ...
                // if($now_lapse >= $lapse_time){ ?>
                  ...
                  if($document_47 && $document_48 && $document_49 && $branch_info->status != 48 && $branch_info->status <= 49) {
                    echo '<a href="'.base_url().'bns_transfer/'.$encrypted_id.'/evaluation_for_submission" class="btn btn-color-blue btnFinalize btn-sm" >Submit</a>';
                  } else {
                    $disabled = '';
                  }?>
                <?php
          ...
          <li class="list-group-item  flex-column align-items-start">
            <div class="d-flex w-100 justify-content-between">
              <h5 class="mb-1 font-weight-bold">Step 5 & 6</h5>
              <small class="text-muted">
            ...
            <?php if(($branch_info->status==51) && $document_47 && $document_48 && $document_49): ?>
              <small class="text-muted">
                <a href="<?php echo base_url();?>for_transfer/<?= $encrypted_id ?>/payments_branch_for_transfer" class="btn btn-color-blue btn-sm ">Payment</a>
              </small>
            <?php endif; ?>
