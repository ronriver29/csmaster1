...
<body>
    <img class="center" src="<?=APPPATH?>../assets/img/header_ooc.png" style="background-repeat: no-repeat; background-position: center;"><br><br><br><br><br><br>
  	<div class="row">
  		...
  		<div class="col-md-4">
  			<center><b>ORDER OF CLOSURE</b></center>
  		</div>
  		<div class="col-md-4">
  		</div>
  	</div>
  	<div class="row">
  		...
  		<div class="col-md-4">
  			<center>(of <?=$branch_info->region?> <?=$branch_info->type?> Office)</center>
  		</div>
  		<div class="col-md-4">
  		</div>
  	</div><br><br>
	...
	<table width=100%>
		<tr>
			<?php if($branch_info->noStreet==null) $x=''; else $x=', ';?>
		...
		<tr>
			<td colspan="2" style="text-align: justify !important; text-indent: 40px;"><b>WHEREAS, </b>this cooperative applied for and was issued Letter of Authority No. <?=$branch_info->certNo?> for the establishment of a <?=$branch_info->type?> Office at <?= ucwords($branch_info->noStreet)?> <?= ucwords($branch_info->st).$x?> <?= $branch_info->brg?> <?= $branch_info->municipality?>, <?= $branch_info->provins?>, <?= $branch_info->regun?>  dated <?=date("F d, Y", strtotime($branch_info->dateRegistered)); ?>.</td>
		</tr>
		<tr>
			<td><i style="color:white;">....</i></td>
		</tr>
		...
		?>
		<tr>
			<td colspan="2" style="text-align: justify !important; text-indent: 40px;"><b>NOW, THEREFORE</b>, by virtue of the powers and duties vested in me by law, I hereby Order the Closure of <?=$brancharea?> <?=$branch_info->type?> Office of the <?=$branch_info->coopName?> and the Cancellation of the Letter of Authority under <?=$branch_info->certNo?> dated <?=date("F d, Y", strtotime($branch_info->date_closure)); ?>.</td>
		</tr>
		<tr>
			<td><i style="color:white;">....</i></td>
		</tr>
		<tr>
			<td colspan="2" style="text-align: justify !important; text-indent: 40px;">Done this <?=date("F d, Y", strtotime($branch_info->ok_for_closure)); ?> in Quezon City, Metro Manila, Philippines.</td>
		</tr>
		<tr>
			<td><i style="color:white;">....</i></td>
		</tr>
	...
	<table width=100%>
		<tr>
			...
			<td><center><b><?=$director?></b><br><i>Regional Director</i></center></td>
		</tr>
	</table>
	...
	<?php // echo '<pre>'; echo print_r($branch_info); echo '</pre>';?>
</body>
