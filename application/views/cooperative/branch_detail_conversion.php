      ...
      <div class="card-body">
        ...
        <?php if($is_client) : ?>
          ...
          <p class="text-muted">
            ...
            <?php if($branch_info->status==55) echo "LETTER SUBMISSION SUCCESSFUL"; ?>
            <?php if($branch_info->status==56) echo "LETTER SUBMISSION SUCCESSFUL"; ?>
            <?php if($branch_info->status==57 && $branch_info->sent_lapse_notif==0) echo "LETTER RECEIVED"; ?>
            <?php if($branch_info->status==57 && $branch_info->sent_lapse_notif==1) echo "FOR UPLOADING"; ?>
            <?php if($branch_info->status==58) echo "FOR VALIDATION"; ?>
            <?php if($branch_info->status==59) echo "FOR VALIDATION"; ?>
            <?php if($branch_info->status==60) echo "FOR EVALUATION"; ?>
            <?php if($branch_info->status==61) echo "FOR EVALUATION"; ?>
            <?php if($branch_info->status==62) echo "DENIED"; ?>
            <?php if($branch_info->status==63) echo "APPROVED. FOR PRINTING AND SUBMISSION OF THE REQUIREMENTS"; ?>
            <?php if($branch_info->status==64) echo "DEFERRED"; ?>
            <?php if($branch_info->status==65) echo "FOR PAYMENT"; ?>
            <?php if($branch_info->status==66) echo "SAVE O.R"; ?>
            <?php if($branch_info->status==67) echo "FOR PRINTING OF COA/LOA"; ?>
            <?php if($branch_info->status==68) echo "ISSUED COA/LOA"; ?>

          </p>
        <?php endif; ?>
      </small>
      </div>
      <?php if(($is_client && ($branch_info->status==21 || $branch_info->status==38 || $branch_info->status==81))): ?>
        <div class="card-footer">
          <a href="<?php echo base_url();?>for_conversion/<?= $encrypted_id ?>/bupdate" class="btn btn-block btn-color-blue"><i class='fas fa-edit'></i> Update Basic Information</a>
        </div>
      <?php endif; ?>
    </div>
  </div>
    ...
    <div class="col-sm-12 col-md-8">
      <ul class="list-group">
        ...
        <li class="list-group-item  flex-column align-items-start">
          ...
          <?php if($branch_info->status!= 0): ?>
            <small class="text-muted">
              <a href="<?php echo base_url();?>branches/<?= $encrypted_id ?>/documents_conversion" class="btn btn-info btn-sm">View</a>
            </small>
          <?php endif;?>
        </li>
          <li class="list-group-item  flex-column align-items-start">
            ...
            <?php if(($branch_info->status == 21 || $branch_info->status == 81) && $document_43): ?>
              <?php if($branch_info->reason != ''): ?>
                <small class="text-muted">
                  <a href="<?php echo base_url();?>bns_conversion/<?= $encrypted_id ?>/evaluate" class="btn btn-color-blue btnFinalize btn-sm ">Submit</a>
                </small>
              <?php else : ?>
            ...
            <li class="list-group-item  flex-column align-items-start">
              <div class="d-flex w-100 justify-content-between">
                ...
                <small class="text-muted">
                  <?php if($document_50 && $document_51 && $document_52 && $document_53) :?>
                  <span class="badge badge-success">COMPLETE</span>
                  <?php endif; ?>
                  <?php if(!$document_50 && !$document_51 && !$document_52 && !$document_53) :?>
                  <span class="badge badge-secondary">PENDING</span>
              ...
              <?php if($branch_info->status == 33 || $branch_info->status == 57 || $branch_info->status >= 63) : ?>
                <small class="text-muted">
                  <?php
                ...
                //if($now_lapse >= $lapse_time){ ?>
                  ...
                  if($branch_info->type == 'Branch'){
                    if($document_50 && $document_51 && $document_52 && ($branch_info->status == 57 || $branch_info->status == 64)) {
                      echo '<a href="'.base_url().'bns_conversion/'.$encrypted_id.'/evaluation_for_submission" class="btn btn-color-blue btnFinalize btn-sm" >Submit</a>';
                    } else {
                      $disabled = '';
                    }
                  } else {
          ...
          <li class="list-group-item  flex-column align-items-start">
            ...
            <?php if(($branch_info->status==65)): ?>
              <small class="text-muted">
                <a href="<?php echo base_url();?>for_conversion/<?= $encrypted_id ?>/payments_branch_for_conversion" class="btn btn-color-blue btn-sm ">Payment</a>
              </small>
            <?php endif; ?>
