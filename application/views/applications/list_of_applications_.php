          ...
          <table class="table table-bordered" id="cooperativesTable">
            ...
            <tbody>
              <?php foreach ($list_cooperatives as $cooperative) : ?>
                <tr>
                  <td><?= $cooperative['proposed_name']?> <?= $cooperative['type_of_cooperative']?> <?php if(!empty($cooperative['acronym_name'])){ echo '('.$cooperative['acronym_name'].')';}?> Cooperative <?= $cooperative['grouping']?></td>
                  <?php if(!$is_client) : ?>
                    <td>
                    ...
                    <td>
                      ...
                      <?php else: ?>
                        ...
                        <?php elseif($cooperative['status']==13): ?>
                          ...
                          ?>
                          <input class="btn btn-color-blue offset-md-10" type="button" id="addOff" onclick="showPayment(<?=$cooperative['id']?>,'<?= encrypt_custom($this->encryption->encrypt($cooperative['proposed_name'].' '.$cooperative['type_of_cooperative'].' '.$acronym_name.' Cooperative '.$cooperative['grouping']))?>')" value="Save O.R. No.">
          ...
          <table class="table table-bordered" id="cooperativesTable2">
            ...
            <tbody>
              <?php foreach ($list_cooperatives_registered as $cooperative_registered) : ?>
                <tr>
                  <td><?= $cooperative_registered['proposed_name']?> <?= $cooperative_registered['type_of_cooperative']?> <?php if(!empty($cooperative_registered['acronym_name'])){ echo '('.$cooperative_registered['acronym_name'].')';}?> Cooperative <?= $cooperative_registered['grouping']?>
                  </td>
                  <td>
                  ...
                  <td>
                      <a href="<?php echo base_url();?>cooperatives/<?= encrypt_custom($this->encryption->encrypt($cooperative_registered['id'])) ?>/registration" class="btn btn-info"><i class='fas fa-print'></i> Print Registration</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
