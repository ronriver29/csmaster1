                ...
                <tr>
                  ...
                  <td>
                      ...
                      <?php else : ?>
                        ...
                        else if($branch['status']==55) echo "LETTER INTENT-CONVERT TO ".$branchtype."";
                        else if($branch['status']==56) echo "LETTER INTENT-CONVERT TO ".$branchtype."";
                        // else if($branch['status']==57) echo "LETTER RECEIVED";
                        else if($branch['status']==57 && $branch['sent_lapse_notif'] == 0) echo "LETTER RECEIVED";
                        else if($branch['status']==57 && $branch['sent_lapse_notif'] == 1) echo "FOR UPLOADING";
                        else if($branch['status']==58) echo "APPLICATION FOR CONVERSION TO ".$branchtype."/FOR ASSIGNMENT OF VALIDATOR";
                        else if($branch['status']==59) echo "FOR VALIDATION";
                        else if($branch['status']==60) echo "VALIDATION REPORT CONVERSION-SUBMITTED BY CDS II / FOR EVALUATION";
                        else if($branch['status']==61) echo "APPLICATION FOR CONVERSION SUBMITTED BY SR.CDS / FOR EVALUATION";
                        else if($branch['status']==62) echo "DENIED";
                        else if($branch['status']==63) echo "APPROVED. FOR PRINTING AND SUBMISSION OF THE REQUIREMENTS";
                        else if($branch['status']==64) echo "DEFERRED";
                        else if($branch['status']==65) echo "FOR PAYMENT";
                        else if($branch['status']==66) echo "SAVE O.R";
                        else if($branch['status']==67) echo "FOR PRINTING OF COA/LOA";
                        else if($branch['status']==68) echo "ISSUED COA/LOA";
                      ?>
                    ...
                    <td>
                      <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                        <?php if($branch['status']==9 && $branch['evaluator3']!=0 && $admin_info->access_level==2): ?>
                          ...
                          <?php } ?>
                        <?php endif; ?>
                        <?php  if(($branch['status']>=2 && $branch['status']<17 && $admin_info->access_level == 1) || ($branch['status']>9 && $branch['status']<17 && $admin_info->access_level == 2 || $admin_info->access_level == 3)  && $branch['status']!=8 || ($branch['status']==2 && $admin_info->access_level == 2 || ($branch['status']>9 && $branch['status']<17 && $supervising_ && $admin_info->access_level==4)) || $branch['status'] == 55 || $branch['status'] == 59 || $branch['status'] == 67 || $branch['status'] == 68 || $branch['status'] >= 60 && $branch['status'] <= 61 && $branch['status'] != 57) : ?>
                          <?php if(!in_array($branch['rtype'],$typearr) || $admin_info->region_code == '00' || $branch['status'] == 68){?>
                            <a href="<?php echo base_url();?>branches/<?= encrypt_custom($this->encryption->encrypt($branch['id'])) ?>/documents_conversion" class="btn btn-info"><i class='fas fa-eye'></i> View Document</a>
                          <?php } ?>
                          
                        <?php elseif($branch['status']==58): ?>
                          <?php if(!in_array($branch['rtype'],$typearr) || $admin_info->region_code == '00'){?>
                            <a href="<?php echo base_url();?>branches/<?= encrypt_custom($this->encryption->encrypt($branch['id'])) ?>/assign" data-toggle="modal" data-target="#assignBranchSpecialistModal" data-coopid="<?= encrypt_custom($this->encryption->encrypt($branch['id']))?>" data-cname="<?=$brancharea.' '?><?= $branch['branchName']?>" class="btn btn-color-blue"><i class='fas fa-user-check'></i> Assign to Validator</a>
                          <?php } ?>
                        <?php elseif($branch['status']==63): ?>
                          <?php if(!in_array($branch['rtype'],$typearr) || $admin_info->region_code == '00'){?>
                            <a href="<?php echo base_url();?>branches/<?= encrypt_custom($this->encryption->encrypt($branch['id'])) ?>/forpaymentbranches_for_conversion" class="btn btnOkForPayment btn-color-blue"> OK For Payment</a>
                            <a href="<?php echo base_url();?>branches/<?= encrypt_custom($this->encryption->encrypt($branch['id'])) ?>/documents_conversion" class="btn btn-info"><i class='fas fa-eye'></i> View Document</a>
                          <?php } ?>

                        <?php elseif($branch['status']==66): ?>
                        ...
                        <?php if($branch['status']==67 || $branch['status']==68): ?>
                          <?php if(!in_array($branch['rtype'],$typearr) || $admin_info->region_code == '00'){?>
                            &nbsp;<a href="<?php echo base_url();?>branches/<?= encrypt_custom($this->encryption->encrypt($branch['id'])) ?>/registration_conversion" class="btn btn-info"><i class='fas fa-print'></i> Print COA / LOA</a>
                          <?php } ?>
                        <?php endif; ?>

                        <?php if($branch['status']==59 && $admin_info->access_level == 2): ?>
...
<script type="text/javascript">
  ...
  function showPayment(coop_id,coop_name) {
    ...
    $('.help-block').empty();
    $.ajax({
        url : "<?php echo base_url('for_conversion/payment')?>/" + coop_id,
        type: "GET",
        dataType: "JSON",
  ...
  function save(){
    ...
    if (x==''){
      ...
      $.ajax({
          url : "<?php echo base_url('for_conversion/saveORConversion')?>/" + 'fuck',
          type: "POST",
          ...
          success: function(data)
          {
              if(data.status) //if success close modal and reload ajax table
              {
                  ...
                  $('#paymentForm')[0].reset();
                  window.location.href="<?php echo base_url('for_conversion')?>";
              }
              else
              {
