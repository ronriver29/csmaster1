...
<div class="container-fluid text-monospace">
  ...
  <div class="row">
    <div class="col-sm-12 col-md-12 text-left">
      <?php
        if($bylaw_info_orig->kinds_of_members!= $bylaw_info->kinds_of_members)
        {
      ...
      <ol class="text-justify" type="1">
        ...
        <?php if(strlen($bylaw_info->additional_requirements_for_membership)>0) :?>
          <?php foreach($members_additional_requirements as $key=> $requirement) : 
            if(empty($members_additional_requirements_orig[$key]))
            {
              $members_additional_requirements_orig[$key]=0;
            }
            $requirement_orig = $members_additional_requirements_orig[$key];
            if($requirement_orig!==$requirement)
            {
  ...
  <div class="row">
    <div class="col-sm-12 col-md-12">
      <?php
       if($bylaw_info_orig->act_upon_membership_days!=$bylaw_info->act_upon_membership_days)
       {
      ...
      $membership_fee2 = ucwords(num_format_custom($bylaw_info->membership_fee));
       ...
       if($bylaw_info_orig->membership_fee!=$bylaw_info->membership_fee)
       {
...
$minimum_paid_up_share_associate_3 ='';
      ...
      <ol class="text-justify" type="1">
        ...
        <?php if(strlen($bylaw_info->additional_conditions_to_vote)>0) :?>
          <?php foreach ($members_additional_conditions_to_vote as $key=> $condition) :
            if(empty($members_additional_conditions_to_vote_orig[$key]))
            {
  ...
  <div class="row">
    <?php
    if($bylaw_info_orig->annual_regular_meeting_day!=$bylaw_info->annual_regular_meeting_day)
    {
  ...
  <div class="row">
    <?php
     $members_percent_quorom=$bylaw_info->members_percent_quorom;
      $members_percent_quorom2 =$bylaw_info->members_percent_quorom;
    if($bylaw_info_orig->members_percent_quorom!=$bylaw_info->members_percent_quorom)
    {
      $members_percent_quorom= '<strong>'. $members_percent_quorom.'</strong>';
       $members_percent_quorom2= '<strong>'.ucwords(num_format_custom($members_percent_quorom2)).'</strong>';
    }
    else
    {
      $members_percent_quorom2= num_format_custom($bylaw_info->members_percent_quorom);
    }
    ?>
    <div class="col-sm-12 col-md-12 text-left">
        <p class="text-justify font-weight-regular">Section 8. <i class="font-weight-bold">Quorum for General Assembly Meeting.</i> During regular or special general assembly meeting, at least <?= $members_percent_quorom2?> percent (<?=  $members_percent_quorom?>%) of the total number of members entitled to vote shall constitute a quorum. </p>
    </div>
  </div>
  <div class="row mb-4">
    <div class="col-sm-12 col-md-12 text-left">
  ...
  <div class="row">
    <?php
    if($bylaw_info_orig->number_of_absences_disqualification !=$bylaw_info->number_of_absences_disqualification)
    {
    ...
    if($bylaw_info_orig->percent_of_absences_all_meettings!=$bylaw_info->percent_of_absences_all_meettings)
    {
  ...
  <div class="row">
    ...
    if($bylaw_info_orig->director_hold_term!=$bylaw_info->director_hold_term)
    {
  ...
  <div class="row">
    ...
    if($bylaw_info_orig->percent_reserve_fund!=$bylaw_info->percent_reserve_fund)
    {
    ...
    if($bylaw_info->percent_education_fund!=$bylaw_info_orig->percent_education_fund)
    {
    ...
    if($bylaw_info_orig->percent_community_fund!=$bylaw_info->percent_community_fund)
    {
    ...
    if($bylaw_info_orig->percent_optional_fund!=$bylaw_info->percent_optional_fund)
    {
  ...
  <div class="row">
    ...
    if($bylaw_info_orig->non_member_patron_years!=$bylaw_info->non_member_patron_years)
    {
    ...
    <div class="col-sm-12 col-md-12">
      <ol class="text-justify" type="a">
        <li>In the case of a member patron with paid-up share capital contribution, his/her proportionate amount of patronage refund shall be paid to him/her unless he/she agrees to credit the amount to his/her account as additional share capital contribution; </li>
    		...
    		<li>In the case of non-member patron, his/her proportionate amount of patronage refunds shall be set aside in a general fund for such patron and shall be allocated to individual non-member patron and only upon request and presentation of evidence of the amount of his/her patronage. The amount so allocated shall be credited to such patron toward payment of the minimum capital contribution for membership. When a sum equal to this amount has accumulated at any time within <?= ($non_member_patron_years)?> (<?=$non_member_patron_years2 ?>) years, such patron shall be deemed and become a member of the Cooperative if he/she so agrees or requests and complies with the provisions of the bylaws for admission to membership; and</li>
  ...
  <div class="row mb-2">
    <?php
    if($bylaw_info_orig->amendment_votes_members_with!=$bylaw_info->amendment_votes_members_with)
    {
