...
<div class="container-fluid text-monospace">
  ...
  <div class="row ">
    <?php
      $totalRegular =  $total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value);
      $total_regular_words=$total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value);
      ...
      $regular_total_subscibed = $total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value) + ($total_associate['total_subscribed'] * stringToNumber($capitalization_info->par_value));
      ...
      $totalRegular2 =($total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value)) + ($total_associate['total_subscribed'] * $capitalization_info->preferred_share);
      $totalRegular2_orig =($total_regular_orig['total_subscribed'] * stringToNumber($capitalization_info->par_value)) + ($total_associate_orig['total_subscribed'] *stringToNumber($capitalization_info->par_value));
      ...
      $paidUp =($total_regular['total_paid'] * stringToNumber($capitalization_info->par_value)) + ($total_associate['total_paid'] * stringToNumber($capitalization_info->par_value));
  ...
  <div class="row mb-2">
    <div class="col-sm-12 col-md-12">
      <div class="table-responsive">
        <table class="table table-sm">
          ...
          <tbody>
            <?php /* $count=0; foreach($regular_cooperator_list as $key => $regular) : ?>
              ...
              ?>
                  ...
                  <td style="text-align: right;"><?= number_format(($regular['number_of_subscribed_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
                  ...
                  <td style="text-align: right;"><?= number_format(($regular['number_of_paid_up_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
              <?php
              }
              else
              {
              ...
              */?>
               ...
               function compares($param1,$param2)
               {
                // global $change_arr;
                // $change_arr = array();
                if($param1!=$param2)
                {
                 $param1 =  '<strong>'.$param1.'</strong>';
                }
                 // $change_arr[] = $param1;
                return $param1;
               }
              $array_fullname = array();
              $array_fullname2 = array();
               $count=0; 
               foreach($regular_cooperator_list as $key => $regular) : ?>
              <tr>
              <?php $count++; ?>
              <?php 
              ...
              {
                  $regular_orig = $regular_cooperator_list_orig[$key];
                  $full_name = compares($regular['full_name'],$regular_orig['full_name']);
                  array_push($array_fullname,$full_name);
              ?>
                  <td><?=$count.'. '. $regular['full_name'].' : '.$regular_orig['full_name']?></td>
                  <td style="text-align: center;">
                    <?= ($regular['number_of_subscribed_shares']!=$regular_orig['number_of_subscribed_shares'] ? '<strong>'.$regular['number_of_subscribed_shares'].'</strong>' : $regular['number_of_subscribed_shares'])?></td>
                  <td style="text-align: right;"><?= number_format(($regular['number_of_subscribed_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
                  <td style="text-align: center;"><?= ($regular['number_of_paid_up_shares']!=$regular_orig['number_of_paid_up_shares'] ? '<strong>'.$regular['number_of_paid_up_shares'].'</strong>' : $regular['number_of_paid_up_shares'])?></td>
              ...
              {
                 $full_name2 = compares($regular['full_name'],$regular_orig['full_name']);
                array_push($array_fullname,$full_name2);
              ?>
                    <td><?=$count.'. '. $regular['full_name']?></td>
                  <td style="text-align: center;"><?= '<strong>'.$regular['number_of_subscribed_shares'].'</strong>'?></td>
                  <td style="text-align: right;"><?= number_format(($regular['number_of_subscribed_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
          ...
          <tfoot>
            <tr>
              ...
              <?php
              $sub =number_format(($total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value)),2);
              $sub_orig= number_format(($total_regular_orig['total_subscribed'] * stringToNumber($capitalization_info_orig->par_value)),2);
              ...
              <?php
             $subs1 = number_format(($total_regular['total_paid'] * stringToNumber($capitalization_info->par_value)),2);
              $subs1_orig = number_format(($total_regular_orig['total_paid'] * stringToNumber($capitalization_info_orig->par_value)),2);
        ...
        $new_array =array_merge($array_fullname , $array_fullname2);// array_merge($array_fullname,$array_fullname2);
        function myfunction($v)
         ...
         array_map("myfunction",$new_array);
        $change_arr[] = $new_array;
        sort($new_array);
        sort($change_arr);
        foreach($change_arr as $r)
        {
          $rowa = $r;
        }
        ...
        <table class="table table-sm">
              ...
              <?php
                if(isset($associate_cooperator_list_orig[$key]))
                {
                  ...
                  ?>
                    ...
                    <td style="text-align: right;"><?= number_format(($associate['number_of_subscribed_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
                ...
                ?>
                     <!-- <td><?='<b>'.$count.'. '. $associate['full_name'].'</b>'?></td> -->
                      ...
                      <td style="text-align: right;"><?= number_format(($associate['number_of_paid_up_shares'] * stringToNumber($capitalization_info->par_value)),2)?></td>
          ...
          <tfoot>
            <tr>
              ...
              <td style="text-align: right;"><?= number_format(($total_associate['total_subscribed'] * stringToNumber($capitalization_info->par_value)),2)?></td>
              <td style="text-align: center;"><?= $total_associate['total_paid'] ?></td>
              <td style="text-align: right;"><?= number_format(($total_associate['total_paid'] * stringToNumber($capitalization_info->par_value)),2)?></td>
            </tr>
            <tr>
              <td>Grand Total</td>
              ...
              ?></td>
              <td style="text-align: right;"><?= number_format((($total_regular['total_subscribed'] * stringToNumber($capitalization_info->par_value)) + ($total_associate['total_subscribed'] * stringToNumber($capitalization_info->par_value))),2)?></td>
              <td style="text-align: center;"><?= $total_regular['total_paid'] + $total_associate['total_paid']?></td>
              <td style="text-align: right;"><?= number_format((($total_regular['total_paid'] * stringToNumber($capitalization_info->par_value)) + ($total_associate['total_paid'] *  stringToNumber($capitalization_info->par_value))),2)?></td>
            </tr>
          </tfoot>
        </table>
      </div>
