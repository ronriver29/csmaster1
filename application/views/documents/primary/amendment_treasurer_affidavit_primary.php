...
<div class="container-fluid text-monospace">
  ...
  <div class="row mb-2">
    <div class="col-sm-12 col-md-12 text-left" style="margin-top: -20px;">
      <p class="text-justify" style="text-indent: 50px;">I, <?= ($treasurer_of_coop_previous!=NULL ?($treasurer_of_coop->full_name!=$treasurer_of_coop_previous->full_name ? "<b>".$treasurer_of_coop->full_name."</b>" : $treasurer_of_coop->full_name) : '__________________')?>, after having been duly sworn to in accordance with law, do hereby depose and say:</p>
  <?php
  ...
  text-justify: inter-word;">
        ...
        <li>That as such, I hereby certify that the authorized share capital of this cooperative is
         <?= ($capitalization_info->authorized_share_capital!=$capitalization_info_previous->authorized_share_capital ? "<b>".ucwords(num_format_custom($capitalization_info->authorized_share_capital))."</b>" : ucwords(num_format_custom($capitalization_info->authorized_share_capital)))?> Pesos (Php 
          <?=$capitalization_info->authorized_share_capital!=$capitalization_info_previous->authorized_share_capital ? "<b>".number_format($capitalization_info->authorized_share_capital,2)."</b>" : number_format($capitalization_info->authorized_share_capital,2);?>)</li>
        <li>That the subscribed share capital of the cooperative is 
          ...
          $subscribed_shared_capitals_previous_inwords =  ucwords(number_format(($bylaw_info->kinds_of_members == 1))) ? str_replace(',','',$total_regular_previous['total_subscribed']) * str_replace(',','',$capitalization_info->par_value) : ucwords($total_regular_previous['total_subscribed'] * $capitalization_info->par_value + $total_associate_previous['total_subscribed'] * $capitalization_info->par_value);
     
         if(strcasecmp($subscribed_shared_capitals_inwords,$subscribed_shared_capitals_previous_inwords)>0)
         {
          ...
          ?>
           ...
           $subscribed_shared_capitals_num_previous =  ($bylaw_info->kinds_of_members == 1) ? number_format((str_replace(',','',($total_regular_previous['total_subscribed']!=NULL ? $total_regular_previous['total_subscribed'] :0)) * str_replace(',','',$capitalization_info->par_value)),2) : number_format(((str_replace(',','',($total_regular_previous['total_subscribed']!=NULL ? $total_regular_previous['total_subscribed'] : 0)) * str_replace(',','',$capitalization_info_previous->par_value)) + (str_replace(',','',$total_associate_previous['total_subscribed']) * str_replace(',','',$capitalization_info_previous->par_value))),2);
           echo ($subscribed_shared_capitals_num!=$subscribed_shared_capitals_num_previous ? "<b>".$subscribed_shared_capitals_num."</b>" : $subscribed_shared_capitals_num);
        ...
        <li>That the paid-up share capital of the cooperative is <?php
          $paid_up = ucwords(number_format(($bylaw_info->kinds_of_members == 1))) ? ucwords(num_format_custom($total_regular['total_paid'] * $capitalization_info->par_value)) : ucwords(num_format_custom($total_regular['total_paid'] * $capitalization_info->par_value + $total_associate['total_paid'] * $capitalization_info->par_value));
           $paid_up_previous = ucwords(number_format(($bylaw_info->kinds_of_members == 1))) ? ucwords(num_format_custom($total_regular_previous['total_paid'] *  $capitalization_info->par_value)) : ucwords(num_format_custom($total_regular_previous['total_paid'] * $capitalization_info->par_value + $total_associate_previous['total_paid'] *  $capitalization_info->par_value));
           echo ($paid_up!=$paid_up_previous ? "<b>".$paid_up."</b>" : $paid_up).' Pesos'; ?>
           (Php 
           <?php 
           $paid_up_num =  ($bylaw_info->kinds_of_members == 1) ? number_format((str_replace(',','',$total_regular['total_paid']) * str_replace(',','', $capitalization_info->par_value)),2) : number_format(((str_replace(',','',$total_regular['total_paid']) * str_replace(',','', $capitalization_info->par_value)) + (str_replace(',','',$total_associate['total_paid']) * str_replace(',','', $capitalization_info->par_value))),2);
             $paid_up_num_previous =  ($bylaw_info->kinds_of_members == 1) ? number_format((str_replace(',','',($total_regular_previous['total_paid']!=NULL ? $total_regular_previous['total_paid'] : 0 )) * str_replace(',','',$capitalization_info->par_value)),2) : number_format(((str_replace(',','',($total_regular_previous['total_paid']!=NULL ? $total_regular_previous['total_paid'] : 0)) * str_replace(',','',$capitalization_info->par_value)) + (str_replace(',','',$total_associate_previous['total_paid']) * str_replace(',','',$capitalization_info->par_value))),2);
            echo ($paid_up_num!= $paid_up_num_previous ? "<b>".$paid_up_num."</b>" : $paid_up_num); 
