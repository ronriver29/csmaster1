  ...
  <div class="row mb-4">
    <div class="col-sm-12 col-md-12">
      <ol class="text-justify" type="1">
        ...
        <li>That as such, I hereby certify that the authorized share capital of this cooperative is <?= ucwords(num_format_custom($article_info->authorized_share_capital)).' Pesos'?> (Php <?= number_format(str_replace(',','',$article_info->authorized_share_capital),2)?>)</li>
        <li>That the subscribed share capital of the cooperative is <?php echo ucwords(number_format(($bylaw_info->kinds_of_members == 1))) ? ucwords(num_format_custom($total_regular['total_subscribed'] * $article_info->par_value_common)).' Pesos' : ucwords(num_format_custom($total_regular['total_subscribed'] * $article_info->par_value_common + $total_associate['total_subscribed'] * $article_info->par_value_preferred)).' Pesos';?> (Php <?php echo ($bylaw_info->kinds_of_members == 1) ? number_format((str_replace(',','',$total_regular['total_subscribed']) * str_replace(',','',$article_info->par_value_common)),2) : number_format(((str_replace(',','',$total_regular['total_subscribed']) * str_replace(',','',$article_info->par_value_common)) + (str_replace(',','',$total_associate['total_subscribed']) * str_replace(',','',$article_info->par_value_preferred))),2);?>) which is at least twenty five (25%) percent of the authorized capital;</li>
        <li>That the paid-up share capital of the cooperative is <!-- <?php echo ucwords(number_format(($bylaw_info->kinds_of_members == 1))).' Pesos' ? ucwords(num_format_custom($total_regular['total_paid'] * $article_info->par_value_common)). ' Pesos' : ucwords(num_format_custom($total_regular['total_paid'] * $article_info->par_value_common + $total_associate['total_paid'] * $article_info->par_value_preferred)). ' Pesos';?> vvvv  -->

          <?php $pd_up='';
            if($bylaw_info->kinds_of_members == 1):
            
             $pd_up = str_replace(',','',$total_regular['total_paid']) * str_replace(',','',$article_info->par_value_common);
            
            else :
            
               $pd_up = str_replace(',','',$total_regular['total_paid']) * str_replace(',','',$article_info->par_value_common) + str_replace(',','',$total_associate['total_paid']) * str_replace(',','',$article_info->par_value_preferred);
            endif;
            echo ucwords(num_format_custom($pd_up,2)).' Pesos';
           ?>
         (Php <?php echo ($bylaw_info->kinds_of_members == 1) ? number_format((str_replace(',','',$total_regular['total_paid']) * str_replace(',','',$article_info->par_value_common)),2) : number_format(((str_replace(',','',$total_regular['total_paid']) * str_replace(',','',$article_info->par_value_common)) + (str_replace(',','',$total_associate['total_paid']) * str_replace(',','',$article_info->par_value_preferred))),2);?>) which is at least twenty five (25%) percent of the subscribed capital; and</li>
