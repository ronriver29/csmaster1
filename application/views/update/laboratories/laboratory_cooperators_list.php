...
<?php if(($is_client && $coop_info->status!=2 && ($is_client && $coop_info->status!=12) || ($is_client && $coop_info->status==24)) && ($coop_info->status!=30) && ($coop_info->status!=31)): ?>
  <div class="col-sm-12 offset-md-8 col-md-4 mb-2">
    <a class="btn btn-color-blue btn-block" role="button" href="<?php echo base_url();?>laboratories_update/<?= $encrypted_id ?>/laboratories_cooperators_update/add" role="button"><i class="fas fa-plus"></i> Add Member/Cooperator
    </a>
  </div>
<?php endif; ?>
<?php if($this->session->userdata('access_level')==6 && ($coop_info->status == 30 ||  $coop_info->status == 31)): ?>
  <div class="col-sm-12 offset-md-8 col-md-4 mb-2">
    <a class="btn btn-color-blue btn-block" role="button" href="<?php echo base_url();?>laboratories_update/<?= $encrypted_id ?>/laboratories_cooperators_update/add" role="button"><i class="fas fa-plus"></i> Add Member/Cooperator
    </a>
  </div>
                  ...
                  <td>
                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                     <!--  <button type="button" class="btn btn-info" data-fname="<?=$cooperator['full_name']?>" data-placeissuance="<?= $cooperator['place_of_issuance']?>" data-dateissued="<?= $cooperator['proof_date_issued']?>" data-valididno="<?= $cooperator['proof_of_identity_number']?>" data-validid="<?= $cooperator['proof_of_identity']?>" data-membertype="<?= $cooperator['type_of_member']?>" data-pos="<?= $cooperator['position']?>" data-paddress="<?= $cooperator['addrCode']?>" data-bdate="<?=$cooperator['birth_date']?>" data-gender="<?=$cooperator['gender']?>" data-toggle="modal" data-target="#fullInfoCooperatorModal" ><i class='fas fa-eye'></i> View</button> -->
                      <?php if(($is_client && $coop_info->status!=2) && ($is_client && $coop_info->status!=12) && $coop_info->status != 30): ?>
                        <a href="<?php echo base_url();?>laboratories_update/<?= $encrypted_id ?>/laboratories_cooperators_update/<?= encrypt_custom($this->encryption->encrypt($cooperator['id'])) ?>/edit" class="btn btn-warning text-white"><i class="fas fa-edit"></i> Edit</a>
