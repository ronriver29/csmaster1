  ...
  <?php else :?>
    ...
    <?php if($admin_info->access_level == 3 && $is_active_director || $supervising_) {?>
     <?php if($coop_info->status !=15):?>
        ...
        <?php } else {?>
          <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#denyCooperativeModal" data-cname="<?= $coop_info->proposed_name?> Federation of <?= $coop_info->type_of_cooperative?> Cooperative <?php if(!empty($coop_info->acronym_name)){ echo '('.$coop_info->acronym_name.')';}?>" data-coopid="<?= encrypt_custom($this->encryption->encrypt($coop_info->id))?>" <?php if($coop_info->tool_yn_answer==null) echo 'disabled';?> >Deny</button>
          <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#deferCooperativeModal" data-cname="<?= $coop_info->proposed_name?> Federation of <?= $coop_info->type_of_cooperative?> Cooperative  <?php if(!empty($coop_info->acronym_name)){ echo '('.$coop_info->acronym_name.')';}?>" data-coopid="<?= encrypt_custom($this->encryption->encrypt($coop_info->id))?>" <?php if($coop_info->tool_yn_answer==null) echo 'disabled';?>>Defer</button>
        <?php } ?>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <?php else: ?>
                ...
                <?php $url_= base_url().'cooperatives/'.$encrypted_id.'/documents/articles_cooperation_federation?id2='.$encrypted_id;?>
        <?php endif; ?>
            ...
            <p class="card-text">
              <?php if(isset($document_one)) : ?>
                 ...
                 <a target="_blank" href="<?php echo base_url();?>documents/list_upload_pdf/<?=$encrypted_id?>/1">
                  ...
                  <?php if($is_client) : ?>
                    This is your Feasibility Study document.
                  <?php else : ?>
                    This is the Feasibility Study document.
                  <?php endif;?>
                </a>
              <?php endif ?>
              <?php if(!isset($document_one)) : ?>
                Please upload your required Feasibility Study document.
              <?php endif ?>
              <br>
            </p>
          ...
          <h5 class="card-title">GA Resolution of each member-cooperative</h5>
            <p class="card-text">
              ...
              <a target="_blank" href="<?php echo base_url();?>documents/list_upload_pdf/<?=$encrypted_id?>/2">
                ...
                <?php else : ?>
                  This is the GA Resolution of each member-cooperative document.
                <?php endif;?>
               </a>
              <?php endif ?>
              <?php if(!isset($document_two)) : ?>
              Please upload your required GA Resolution of each member-cooperative document
              <?php endif ?>
              <br>
            </p>
          ...
          <h5 class="card-title">BOD resolution on authorized representative of each of the member-cooperatives</h5>
            <p class="card-text">
              ...
              <a target="_blank" href="<?php echo base_url();?>documents/list_upload_pdf/<?=$encrypted_id?>/3">
                ...
                <?php else : ?>
                  This is the BOD resolution on authorized representative of each of the member-cooperatives document.
                <?php endif;?>
               </a>
              <?php endif ?>
              <?php if(!isset($document_three)) : ?>
              Please upload your required BOD resolution on authorized representative of each of the member-cooperatives document
              <?php endif ?>
              <br>
            </p>
          <?php if($is_client && $coop_info->status<=1 || $coop_info->status==11): ?>
            <a href="<?php echo base_url();?>cooperatives/<?=$encrypted_id?>/documents/upload_document_three" class="btn btn-primary">Upload</a>
          <?php endif; ?>
        </div>
      </div>
  </div>
  ...
  <div class="col-sm-12 col-md-4">
      <div class="card">
        <div class="card-body">
          ...
          <?php if($is_client && $coop_info->status<=1 || $coop_info->status==11): ?>
            <?php if($is_client) : ?>
                <a href="<?php echo base_url();?>cooperatives/<?=$encrypted_id?>/documents/upload_document_unifed_sbao" class="btn btn-primary">Upload</a>
            <?php endif;?>
          <?php endif; ?>
