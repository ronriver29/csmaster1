...
use Dompdf\Options;
class Documents_conversion extends CI_Controller{
   var $pageCount = array();
...
public function list_upload_pdf_branch($id =null,$doc_type=null)
{
  ...
  else
  {
    ...
    if($data['is_client'] ==1)
    {
      ...
      $this->load->view('template/header',$data);
      $this->load->view('documents/list_of_uploaded_pdf_branch_conversion',$data);
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            if($this->branches_model->check_own_branch($decoded_id,$user_id)){
                ...
                $data['document_50'] = $this->uploaded_document_model->get_document_50_info($decoded_id,$branch_info->application_id);
                $data['document_51'] = $this->uploaded_document_model->get_document_51_info($decoded_id,$branch_info->application_id);
                $data['document_52'] = $this->uploaded_document_model->get_document_52_info($decoded_id,$branch_info->application_id);
                $data['document_53'] = $this->uploaded_document_model->get_document_53_info($decoded_id,$branch_info->application_id);
                $data['document_54'] = $this->uploaded_document_model->get_document_54_info($decoded_id,$branch_info->application_id);
...
//                if($this->branches_model->check_submitted_for_evaluation($decoded_id)) {
                  ...
                  // $data['last_query'] = $this->db->last_query();
                  // New
                  $data['branches_comments_cds'] = $this->branches_model->branches_comments_cds_conversion($branch_info->id);
                  $data['branches_comments_snr'] = $this->branches_model->branches_conversion_comments_snr($branch_info->id,$evaluator_number);
                  $data['branches_comments_snr_limit'] = $this->branches_model->branches_comments_snr_conversion_limit($branch_info->id,$evaluator_number);
                  //
                ...
                if($data['branch_info']->area_of_operation == 'Interregional'){
                  ...
                  $this->load->view('templates/admin_header', $data);
                  $this->load->view('documents/list_of_documents_branch_conversion', $data);
                  $this->load->view('cooperative/evaluation/approve_modal_branch_for_conversion');
                  $this->load->view('cooperative/evaluation/deny_modal_branch_for_conversion');
                  $this->load->view('cooperative/evaluation/defer_modal_branch_for_conversion');
                  $this->load->view('templates/admin_footer');
//                } else{
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_letter_of_intent_conv', $data);
                $this->load->view('./template/footer');
          }else{
            echo $decoded_id.'-'.$user_id;
            // $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!!!!.');
