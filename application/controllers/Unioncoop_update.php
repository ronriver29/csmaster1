...
class Unioncoop_update extends CI_Controller{
...
//                    $data['list_cooperators_count'] = $this->cooperator_model->get_all_cooperator_of_coop_regular_count($decoded_id);
                    ...
                    $this->load->view('update/union/union_list', $data);
                    $this->load->view('federation/full_info_modal_registeredcoop');
                    $this->load->view('update/union/add_form_affiliates');
                    $this->load->view('update/union/edit_form_cooperator');
                    $this->load->view('update/union/delete_form_affiliates');
...
//                        $data['minimum_associate_pay'] = $capitalization_info->minimum_paid_up_share_associate;
    ...
    function add_affiliates($id = null){
        ...
        if(!$data['is_client']){
          $data['coop_info'] = $this->cooperatives_update_model->get_cooperative_info_by_admin($decoded_post_coop_id);
        ...
        if($count==1){
          $this->session->set_flashdata('cooperator_error', 'Position already exists.');
            redirect('cooperatives_update/'.$encrypted_post_coop_id.'/unioncoop_update');
        } else {
          if($query==0){
              $data = array(
  ...
  //            echo $query;
    ...
    function edit_unioncoop($id = null){
        ...
        if($count==1){
          $this->session->set_flashdata('cooperator_error', 'Position already exists.');
            redirect('cooperatives_update/'.$encryptedcoopid.'/unioncoop_update');
        } else {
          $u_data = array(
    ...
    function update_cc($id = null){
      if(!$this->session->userdata('logged_in')){
        ...
        if($this->input->post('addAdministratorBtn')){
            ...
            $update_aff = $this->db->update('cooperatives',$u_data,array('id'=>$decoded_post_coop_id));
...
//              if($this->cooperator_model->check_cooperator_in_cooperative($decoded_id,$decoded_post_cooperator_id)){
                // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                  ...
                  if($success){
                    ...
                    redirect('cooperatives_update/'.$this->input->post('cooperativeID').'/unioncoop_update');
                  }else{
