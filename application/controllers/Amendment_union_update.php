...
class Amendment_union_update extends CI_Controller{
  public $coopName = '';
  public $regNo='';
  ...
  function index($id = null)
  {
      ...
      redirect('users/login');
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            ...
            $data['requirements_complete'] = $this->union_model->is_requirements_complete($user_id);
            $data['cc_count'] = $this->union_model->get_total_cc($decoded_id);
            $data['directors_count'] = $this->union_model->check_no_of_directors($decoded_id);
            $data['directors_count_odd'] = $this->union_model->check_directors_odd_number($user_id);
            $data['total_directors'] = $this->union_model->no_of_directors($decoded_id);
            $data['chairperson_count'] = $this->union_model->check_chairperson($decoded_id);
            $data['vice_count'] = $this->union_model->check_vicechairperson($decoded_id);
            $data['treasurer_count'] = $this->union_model->check_treasurer($decoded_id);
            $data['secretary_count'] = $this->union_model->check_secretary($decoded_id);
            $data['msg'] ='';
            $submit =false;
            ...
            // $array =array(
            //       'url'=>base_url()."amendment_update/".$id."/union_update",
            //       'total_rows'=>$this->union_model->registered_coop_count($data['coop_info']->area_of_operation,$data['coop_info']->refbrgy_brgyCode,$data['coop_info']->type_of_cooperative,$this->coopName,$this->regNo),
            ...
            $data['registered_coop'] = $this->union_model->get_registered_interregion($data['coop_info']->regions);
            } else {
            $data['registered_coop'] = $this->union_model->get_registered_coop($data['coop_info']->area_of_operation,$data['coop_info']->refbrgy_brgyCode,$data['coop_info']->type_of_cooperative,$this->regNo,$this->coopName); 
            } 
            $data['msg'] = ($submit && isset($data['registered_coop']['msg']) ? $data['registered_coop']['msg'] :'');
            $data['applied_coop'] = $this->union_model->get_applied_coop($decoded_id);
            ...
            $this->load->view('update/amendment/union_update/union_list', $data);
            $this->load->view('update/amendment/union_update/full_info_modal');
            $this->load->view('update/amendment/union_update/add_form_affiliates');
            $this->load->view('update/amendment/union_update/edit_form_cooperator');
            $this->load->view('update/amendment/union_update/delete_form_affiliates');
            ...
            if($this->session->userdata('access_level')!=6){
              redirect('admins/login');
                ...
                // $data['capitalization_complete'] =$this->amendment_update_capitalization_model->check_capitalization_primary_complete($decoded_id);
                // $data['bylaw_complete'] =$this->amendment_update_bylaw_model->check_bylaw_primary_complete($cooperative_id,$decoded_id);
                $data['title'] = 'List of Members';
                $data['header'] = 'List of Members';
                $data['admin_info'] = $this->admin_model->get_admin_info($this->session->userdata('user_id'));
                $data['encrypted_id'] = $id;
                $data['business_activities'] =  $this->amendment_update_model->get_all_business_activities($decoded_id);
                $data['requirements_complete'] = $this->union_model->is_requirements_complete($decoded_id);
                $data['cc_count'] = $this->union_model->get_total_cc($decoded_id);
                $data['directors_count'] = $this->union_model->check_no_of_directors($decoded_id);
                $data['directors_count_odd'] = $this->union_model->check_directors_odd_number($decoded_id);
                $data['total_directors'] = $this->union_model->no_of_directors($decoded_id);
                $data['chairperson_count'] = $this->union_model->check_chairperson($decoded_id);
                $data['vice_count'] = $this->union_model->check_vicechairperson($decoded_id);
                $data['treasurer_count'] = $this->union_model->check_treasurer($decoded_id);
    ...
    function add_affiliates(){
        ...
        $user_id = $this->session->userdata('user_id');
        $query = $this->union_model->existing_unioncoop($user_id,$this->input->post('regNo'));
        ...
        $data['cc_count'] = $this->union_model->get_total_cc($user_id);
        ...
        if($count==1){
          ...
          if($query==0){
            ...
            else
            {
              ...
              // $this->debug($data); 
              $success = $this->union_model->add_unioncoop($data);
        ...
        if($count==1){
          ...
          {  
            $this->session->set_flashdata('cooperator_success', 'Affiliator Successfully Updated.');
              redirect('amendment_update/'.$encryptedcoopid.'/union_update');
          }else{
...
//              if($this->cooperator_model->check_cooperator_in_cooperative($decoded_id,$decoded_post_cooperator_id)){
                // if(!$this->amendment_update_model->check_submitted_for_evaluation($cooperative_id,$decoded_id)){
                  $success = $this->union_model->delete_affiliators($decoded_post_cooperator_id);
                  if($success){
                    $this->session->set_flashdata('cooperator_success', 'Affiliator has been remove.');
                    redirect('amendment_update/'.$this->input->post('cooperativeID').'/union_update');
                  }else{
