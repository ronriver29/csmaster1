...
use Dompdf\Options;
class Documents_update extends CI_Controller{
   var $pageCount = array();
                              ...
                              // if($data['staff_complete']){
                                ...
                                if($data['coop_info']->grouping == 'Federation'){
                                  ...
                                  $this->load->view('update/documents/list_of_documents_union', $data);
                                    ...
                                    $this->load->view('update/documents/list_of_documents', $data);
                                }
...
public function list_upload_pdf($id =null,$doc_type=null)
{
  ...
  else
  {
    ...
    $data['coop_info'] = $data['coop_info'] = $this->cooperatives_update_model->get_cooperative_info($decoded_id);
    if($data['is_client'] ==1)
    {
     
      $this->load->view('template/header',$data);
      $this->load->view('update/documents/list_of_uploaded_pdf',$data);
      $this->load->view('update/documents/delete_pdf_modal');
      $this->load->view('template/footer');
    }
    if($this->session->userdata('access_level')<=5 && $data['is_client']!=1 || ($this->session->userdata('access_level')==6 && $data['is_client']!=1)){
...
public function delete_pdf()
  {
    ...
    $file = $config['upload_path'].$config['file_name'];
       ...
       if(is_readable($file)){
            ...
            {
              $this->session->set_flashdata('delete_success', 'File Successfully deleted.');
                  redirect('/documents_update/list_upload_pdf/'.$coop_id.'/'.$doc_type);
            }
            else
            {
                $this->session->set_flashdata('delete_error', 'Failed to delete in database.');
      ...
      if(is_numeric($decoded_id) && $decoded_id!=0){
        if($this->session->userdata('client')){
                          ...
                          // if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            // if($data['staff_complete']){
                              // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_one', $data);
                                $this->load->view('./template/footer');
              //                 }else{
          ...
          if($this->session->userdata('access_level')==5){
            ...
            $data['title'] = 'Upload Document';
            $data['header'] = 'Upload Document';
            $data['coop_info'] = $this->cooperatives_update_model->get_cooperative_info2($user_id);
            // echo $this->db->last_query();
            $data['encrypted_id'] = $id;
            $data['encrypted_uid'] = encrypt_custom($this->encryption->encrypt($user_id));
            $data['uid'] = $user_id;
                          ...
                          // if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            // if($data['staff_complete']){
                              // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $data['coopid'] = $decoded_id;
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_two', $data);
                                $this->load->view('./template/footer');
              //                 }else{
              //                   $this->session->set_flashdata('redirect_message', 'You already submitted this for evaluation. Please wait for an e-mail of either the payment procedure or the list of documents for compliance.');
              //                   redirect('cooperatives/'.$id);
                          ...
                          if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            if($data['staff_complete']){
                              if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $data['coopid'] = $decoded_id;
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_three', $data);
                                $this->load->view('./template/footer');
                              }else{
                                $this->session->set_flashdata('redirect_message', 'You already submitted this for evaluation. Please wait for an e-mail of either the payment procedure or the list of documents for compliance.');
                                redirect('cooperatives/'.$id);
                          ...
                          if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            if($data['staff_complete']){
                              if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $data['coopid'] = $decoded_id;
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_unifed_sbao', $data);
                                $this->load->view('./template/footer');
                              }else{
                                $this->session->set_flashdata('redirect_message', 'You already submitted this for evaluation. Please wait for an e-mail of either the payment procedure or the list of documents for compliance.');
                                redirect('cooperatives/'.$id);
                          ...
                          // if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            // if($data['staff_complete']){
                              // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $data['coopid'] = $decoded_id;
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_others_unifed', $data);
                                $this->load->view('./template/footer');
              //                 }else{
              //                   $this->session->set_flashdata('redirect_message', 'You already submitted this for evaluation. Please wait for an e-mail of either the payment procedure or the list of documents for compliance.');
              //                   redirect('cooperatives/'.$id);
                          ...
                          if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            ...
                            if($data['staff_complete']){
                              if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                                ...
                                $data['coopid'] = $decoded_id;
                                $this->load->view('./template/header', $data);
                                $this->load->view('update/upload_form/upload_document_others', $data);
                                $this->load->view('./template/footer');
                              }else{
                                $this->session->set_flashdata('redirect_message', 'You already submitted this for evaluation. Please wait for an e-mail of either the payment procedure or the list of documents for compliance.');
                                redirect('cooperatives/'.$id);
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_5', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_6', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_7', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_40', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_others_bns', $data);
                $this->load->view('./template/footer');
          }else{
        ...
        if($this->session->userdata('client')){
          if($this->laboratories_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->laboratories_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_others_lab', $data);
                $this->load->view('./template/footer');
          }else{
            echo $decoded_id.'-'.$user_id;
            // $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_8', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('update/upload_form/upload_document_9', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
        ...
        if($this->session->userdata('access_level') && $this->session->userdata('access_level')==5){
          ...
          // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            if(!($this->upload->do_upload('file1'))){
              $this->session->set_flashdata('document_one_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('cooperatives_update/'.$this->input->post('cooperativesID').'/documents_update');
            }else{
                if($this->input->post('status')==11){
                    $status = 2;
                } else {
      ...
      if(is_numeric($decoded_id) && $decoded_id!=0){
        if($this->session->userdata('client')){
              ...
              $data['bylaw_complete'] = ($data['coop_info']->category_of_cooperative=="Primary") ? $this->bylaw_model->check_bylaw_primary_complete_amend($decoded_id) : true;
                  ...
                  $data['article_complete'] = ($data['coop_info']->category_of_cooperative=="Primary") ? $this->article_update_model->check_article_primary_complete_amend($decoded_id) : true;
                  $data['committees_complete'] = $this->committee_model->committee_complete_count($decoded_id);
                  $data['economic_survey_complete'] = $this->economic_survey_model->check_survey_complete($decoded_id);
                  $data['bylaw_info'] = $this->bylaw_model->get_bylaw_by_coop_id_amend($decoded_id);
