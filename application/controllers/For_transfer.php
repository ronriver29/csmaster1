...
//              $data['regno'] = $data['coopreg_info']->regNo;
          ...
          $this->load->view('applications/list_of_branches_for_transfer', $data);
          ...
          if($this->session->userdata('access_level')==5){
            ...
            $data['title'] = 'List of Branches and Satellites For Transfer';
            $data['header'] = 'Branches and Satellites For Transfer';
            $data['admin_info'] = $this->admin_model->get_admin_info($user_id);
            ...
            if($this->session->userdata('access_level')==1){
              if($data['admin_info']->region_code=="00"){
                ...
                $data['list_branches'] = $this->branches_model->get_all_branches_by_specialist_for_transfer_ho($data['admin_info']->region_code,$user_id);
                // echo $this->db->last_query();
              }else{
                $data['registered_branches'] = $this->branches_model->get_registered_branches($data['admin_info']->region_code);
                $data['list_branches'] = $this->branches_model->get_all_branches_by_specialist_for_transfer($data['admin_info']->region_code,$user_id);
              }
            }else if($this->session->userdata('access_level')==2){
              if($data['admin_info']->region_code=="00"){
                $data['registered_branches'] = $this->branches_model->get_registered_branches($data['admin_info']->region_code);
                $data['list_branches'] = $this->branches_model->transferred_region_senior_ho($data['admin_info']->region_code);
                $data['transferred_branch'] = $this->branches_model->get_all_branches_by_senior_for_transfer($data['admin_info']->region_code);
                // echo $this->db->last_query();
                $data['list_specialist'] = $this->admin_model->get_all_specialist_by_region_ho('00');
                // echo $this->db->last_query();
              } else {
                $data['registered_branches'] = $this->branches_model->get_registered_branches($data['admin_info']->region_code);
                $data['list_branches'] = $this->branches_model->transferred_region_senior($data['admin_info']->region_code);
                $data['transferred_branch'] = $this->branches_model->get_all_branches_by_senior_for_transfer($data['admin_info']->region_code);
                // echo $this->db->last_query();
              ...
              if($data['admin_info']->region_code=="00"){
                $data['list_branches'] = $this->branches_model->get_all_branches_by_director_for_transfer_ho($data['admin_info']->region_code);
                $data['registered_branches'] = $this->branches_model->get_registered_branches($data['admin_info']->region_code);
                $data['transferred_branch'] = $this->branches_model->get_all_branches_by_director_for_transfer_within($data['admin_info']->region_code);
              } else {
                $data['list_branches'] = $this->branches_model->get_all_branches_by_director_for_transfer($data['admin_info']->region_code);
                $data['registered_branches'] = $this->branches_model->get_registered_branches($data['admin_info']->region_code);
                $data['transferred_branch'] = $this->branches_model->get_all_branches_by_director_for_transfer_within($data['admin_info']->region_code);
              }
            }
            ...
            $data['date2']  = $date_;
            $this->load->view('applications/list_of_branches_for_transfer', $data);
            $this->load->view('applications/assign_branch_transfer_modal');
    ...
    public function payment($coop){
      ...
      $data = $this->branches_model->get_payment_info_transfer($coop);
      echo json_encode($data);
      // print_r($data);
    }

    public function saveORTransfer($was){
      $data = array(
      ...
      $this->branches_model->save_OR_transfer(array('id' => $this->input->post('payment_id')), $data,$this->input->post('bid'),$date_of_or);
      echo json_encode(array("status" => TRUE, "message"=>"O.R. No has been saved."));
      
    }

    public function deny_branch_for_transfer(){
      if(!$this->session->userdata('logged_in')){
        redirect('users/login');
      }else{
                              ...
                              if($this->branches_model->check_evaluator3($decoded_id)){
                                if($this->branches_model->check_evaluator4($decoded_id)){
                                  ...
                                  // }else{
                                    ...
                                    // echo $coop_full_name;
                                    if($this->branches_model->sendEmailToClientDenyTransfer($coop_full_name,$brgyforemail,$reason_commment,$data['client_info']->email,$branch_info->type,$rdregion->regDesc)){
                                      $success = $this->branches_model->deny_by_admin_for_transfer($admin_info,$decoded_id,$reason_commment,1);
                                      if($success){
                                        $this->session->set_flashdata('list_success_message', 'Branch has been denied.');
                                        redirect('for_transfer');
                                      }else{
...
//                      if($this->branches_model->check_not_yet_assigned($decoded_id)){
                      ...
                      if($this->branches_model->sendEmailToSpecialistTransfer($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$emaildirect,$branch_info->type,$coop_region->region)){
                          if($this->branches_model->assign_to_specialist_for_transfer($decoded_id,$decoded_specialist_id,$coop_full_name)){
                      ...
                      if($this->session->userdata('access_level')==4){
                          if($this->branches_model->check_evaluator1($decoded_id)){
                            if($this->branches_model->check_evaluator2($decoded_id)){
                              if($this->branches_model->check_evaluator3($decoded_id)){
                                if($this->branches_model->check_evaluator4($decoded_id)){
                                  if($this->branches_model->check_evaluator_for_transfer2($decoded_id)){
                                    $this->session->set_flashdata('redirect_applications_message', 'Branch already evaluated by the Regional Director.');
                                    redirect('for_transfer');
                                  }else{
                                    $regioncode = "0".mb_substr($branch_info->addrCode, 0, 2);
                            ...
                            if($this->branches_model->check_evaluator2($decoded_id)){
                              if($this->branches_model->check_evaluator3($decoded_id)){
                                if($this->branches_model->check_evaluator4($decoded_id)){
                                  ...
                                  $success = $this->branches_model->insert_transfer_comment_history($data_field);
                                   $coop_full_name = $this->input->post('bName',TRUE);
                                   $reason_commment ='';
                                   ...
                                   if(($branch_info->status == 23 || $branch_info->status == 15) && $branch_info->regCode != 0){
                                      $seniorregionname = $this->branches_model->get_region_name($branch_info->transferred_region);
                                    } else {
                                      $seniorregionname = $this->branches_model->get_region_name($regioncode);
                                    }
                                   ...
                                   $brgyforemail = ucwords($branch_info->house_blk_no).' '.ucwords($branch_info->street).$x.' '.$branch_info->brgy.', '.$branch_info->city.', '.$branch_info->province.', '.$branch_info->region;
                                    ...
                                    // }
                                      if($branch_info->status == 42){
                                        $step = 11;
                                      } else if($branch_info->status == 47){
                                        $step = 13;
                                      }
                                      ...
                                      if($step == 13){
                                        $this->branches_model->sendEmailToClientApproveBranchTransfer($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$senior_info,$branch_info->type,'',$seniorregionname->regDesc);
                                      }
                                      if($step == 11){
                                        $this->branches_model->sendEmailToClientReceivedBranchTransfer($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$senior_info,$branch_info->type,'',$seniorregionname->regDesc);
                                      }
                                    $success = $this->branches_model->approve_by_admin2_for_transfer($admin_info,$decoded_id,$reason_commment,$step,$comment_by_specialist_senior, $coop_full_name);
                                    if($success){
                                      $this->session->set_flashdata('list_success_message', 'Branch/Satellite has been approved.');
                                      redirect('for_transfer');
                                    }else{
...
//                                }else{
                                  ...
                                  //     $regioncode = "0".mb_substr($branch_info->transferred_region, 0, 2);
                                  //   } else {
                                  //     $regioncode = "00";
                                  //   }
                                  // } else {
                                  ...
                                  if($this->db->count_all_results()>0)
                                  {
                                    if($branch_info->status == 24 && $branch_info->regCode != 0){
                                      $data['director_info'] = $this->admin_model->get_emails_of_director_by_region($branch_info->transferred_region);
                                    } else {
                                      if($branch_info->status == 46){
                                        $emaildirect = $this->admin_model->get_emails_of_director_by_region('0'.mb_substr($branch_info->transferred_region, 0, 2));
                                      } else {
                                        $emaildirect = $this->admin_model->get_emails_of_director_by_region_transfer('0'.mb_substr($branch_info->addrCode, 0, 2),'0'.mb_substr($branch_info->transferred_region, 0, 2));
                                      }
                                      // $data['director_info'] = $this->admin_model->get_emails_of_director_by_region_transfer("0".mb_substr($branch_info->transferred_region, 0, 2),$regioncode);
                                      // echo print_r($data['director_info']);
                                    }
                                  } else {
                                    if($branch_info->status == 41){
                                      $emaildirect = $this->admin_model->get_emails_of_director_by_region_transfer('0'.mb_substr($branch_info->addrCode, 0, 2),'0'.mb_substr($branch_info->transferred_region, 0, 2));
                                  ...
                                  if($step == 14){
                                    if($this->branches_model->sendEmailToDirectorTransferForApproval($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$emaildirect,$branch_info->type,$fullnamecds,$coop_region->region)){
                                      $success = $this->branches_model->approve_by_admin2_for_transfer($admin_info,$decoded_id,$reason_commment,$step,$comment_by_specialist_senior,$coop_full_name);
                                    }
                                  } else {
                                    if($this->branches_model->sendEmailToDirectorTransfer($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$emaildirect,$branch_info->type,$fullnamecds,$coop_region->region)){
                                      $success = $this->branches_model->approve_by_admin2_for_transfer($admin_info,$decoded_id,$reason_commment,$step,$comment_by_specialist_senior,$coop_full_name);
                                    }
                                  }
                          ...
                          if($this->branches_model->check_evaluator1($decoded_id)){
                            if($this->branches_model->check_evaluator2($decoded_id)){
                             ...
                             $coop_full_name = $this->input->post('bName',TRUE);
                                ...
                                $coop_region = $this->branches_model->get_coop_region($branch_info->regNo);
                                // echo $coop_full_name.'-'.$branchname.'-'.$brgyforemail.'-'.$fullnameforemail.'-'.$data['client_info']->contact_number.'-'.$data['client_info']->email.'-'.$emaildirect.'-'.$branch_info->type;
                                if($this->branches_model->sendEmailToSeniorFromCDSBranchTransfer($branch_info->coopName,$branchname,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$seniorinfo,$branch_info->type,$data['suvervising_info']->full_name,$coop_region->region)){
                                  $success = $this->branches_model->approve_by_admin2_for_transfer($admin_info,$decoded_id,$reason_commment,12,$comment_by_specialist_senior,$coop_full_name);
                                  if($success){
                                    $this->session->set_flashdata('list_success_message', 'Branch/Satellite has been submitted.');
                                    redirect('for_transfer');
                                  }else{
                        ...
                        if($this->session->userdata('access_level')==4){
                          ...
                          if($this->branches_model->check_evaluator1($decoded_id)){
                            if($this->branches_model->check_evaluator2($decoded_id)){
                              if($this->branches_model->check_evaluator3($decoded_id)){
                                if($this->branches_model->check_evaluator4($decoded_id)){
                                  // if($this->branches_model->check_evaluator_for_transfer($decoded_id)){
                                  //   $this->session->set_flashdata('redirect_applications_message', 'Branch already evaluated by the Regional Director.');
                                  //   redirect('for_transfer');
                                  // }else{
                                      $data_field = array(
                                  ...
                                  if($data['branch_info']->status == 47){
                                    ...
                                    if($this->branches_model->sendEmailToClientDeferBranchTransfer($proposednameemail,$proposedbranch,$brgyforemail,$fullnameforemail,$data['client_info']->contact_number,$data['client_info']->email,$data['senior_info']->email,$data['branch_info']->type,$reason_commment,$data['admin_info']->region_code,$reg_officials_info,$rdregion->regDesc)){
                                      if($success){
                                        $this->session->set_flashdata('list_success_message', 'Branch/Satellite has been deferred.');
                                        redirect('for_transfer');
                                      }else{
