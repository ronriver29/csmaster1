...
class Affiliators_update extends CI_Controller{
...
//                    $data['minimum_associate_pay'] = $this->cooperator_model->check_all_minimum_associate_pay($decoded_id);
                    if(isset($capitalization_info->minimum_subscribed_share_regular) && isset($capitalization_info->minimum_paid_up_share_regular) && isset($capitalization_info->minimum_subscribed_share_associate) && isset($capitalization_info->minimum_paid_up_share_associate)){
                      ...
                      $data['minimum_regular_subscription'] = 0;
                      $data['minimum_regular_pay'] = 0;
                      $data['minimum_associate_subscription'] = 0;
                      $data['minimum_associate_pay'] = 0;
                    }
...
//                    $data['list_cooperators_count'] = $this->cooperator_model->get_all_cooperator_of_coop_regular_count($decoded_id);
                    ...
                    $this->load->view('update/federation/affiliators_list', $data);
                    $this->load->view('update/federation/full_info_modal_registeredcoop');
                    $this->load->view('update/federation/add_form_cooperator', $data);
                    $this->load->view('update/federation/edit_form_cooperator', $data);
                    $this->load->view('update/federation/delete_form_cooperator');
...
//                        $data['secretary_count'] = $this->cooperator_model->check_secretary($decoded_id);
                        ...
                        $this->load->view('update/federation/affiliators_list', $data);
                        $this->load->view('update/federation/full_info_modal_registeredcoop');
                        $this->load->view('update/federation/add_form_cooperator');
                        $this->load->view('update/federation/edit_form_cooperator');
                        $this->load->view('update/federation/delete_form_cooperator');
                        $this->load->view('./template/footer');
                  }else{
    ...
    function add_affiliators($id = null){
        ...
        if($found=='found'){
          $this->session->set_flashdata('cooperator_error', 'Position already exists.');
            redirect('cooperatives_update/'.$encrypted_post_coop_id.'/affiliators_update');
        } else {
          if($query==0){
  ...
  //            echo $query;
    ...
    function edit_affiliators($id = null){
        ...
        if($found=='found'){
          $this->session->set_flashdata('cooperator_error', 'Position already exists.');
            redirect('cooperatives_update/'.$encryptedcoopid.'/affiliators_update');
        }
...
//              if($this->cooperator_model->check_cooperator_in_cooperative($decoded_id,$decoded_post_cooperator_id)){
                // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                  ...
                  if($success){
                    ...
                    redirect('cooperatives_update/'.$this->input->post('cooperativeID').'/affiliators_update');
                  }else{
