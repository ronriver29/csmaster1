...
use Dompdf\Options;
class Documents_transfer_submission extends CI_Controller{
   var $pageCount = array();
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            if($this->branches_model->check_own_branch($decoded_id,$user_id)){
                ...
                $data['document_47'] = $this->uploaded_document_model->get_document_47_info($branch_info->id,$branch_info->application_id);
                $data['document_48'] = $this->uploaded_document_model->get_document_48_info($branch_info->id,$branch_info->application_id);
                $data['document_49'] = $this->uploaded_document_model->get_document_49_info($branch_info->id,$branch_info->application_id);
                ...
                $this->load->view('template/header', $data);
                $this->load->view('documents/list_of_documents_branch_transfer', $data);
                $this->load->view('template/footer');
...
//          }
  ...
  function upload_document_for_transfer_47($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_transfer_47', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_transfer_48($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_transfer_48', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_transfer_49($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_transfer_49', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file47'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_transfer_submission');
            }else{
                if($this->input->post('status')==17){
                    $status = 2;
                } else {
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,47,$this->upload->data('file_name'),$status)){
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file48'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_transfer_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,48,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_transfer_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file49'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_transfer_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,49,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_transfer_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
