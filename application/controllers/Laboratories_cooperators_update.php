...
class Laboratories_cooperators_update extends CI_Controller{
...
//                if($data['bylaw_complete']){
                    ...
                    $this->load->view('update/laboratories/laboratory_cooperators_list', $data);
                    $this->load->view('cooperators/full_info_modal_cooperator');
                    $this->load->view('update/laboratories/delete_form_cooperator_laboratory');
...
//                        'number_of_paid_up_shares' =>$this->input->post('paidShares'),
                      ...
                      if($name_check)
                      {
                        ...
                        $this->session->set_flashdata('member_error', "Member/Cooperator Name already exist.");
                         redirect('laboratories_update/'.$this->input->post('cooperativesID').'/laboratories_cooperators_update');
                         //get last name
                      ...
                      else
                      {
                        ...
                        // var_dump($success);
                          if($success['success']){
                            $this->session->set_flashdata('member_success', $success['message']);
                            redirect('laboratories_update/'.$id.'/laboratories_cooperators_update');
                          }else{
                            $this->session->set_flashdata('member_error', $success['message']);
                            redirect('laboratories_update/'.$id.'/laboratories_cooperators_update');
                          }
                      ...
                      $user_id = $this->session->userdata('user_id');
                      $CoopInfo = $this->laboratories_model->get_cooperative_info_updating($decoded_id);
                     ...
                     //cooperative id
                      ...
                      $this->load->view('./template/header', $data);
                      $this->load->view('update/laboratories/add_form_cooperator_laboratory', $data);
                      $this->load->view('./template/footer');
                    }
                // }else{
                ...
                //   redirect('laboratories_update/'.$id);
                // }
              // }else{
              //   redirect('laboratories_update/'.$id);
              // }
                ...
                $data['coop_info'] = $this->laboratories_model->get_cooperative_info_updating($decoded_id);
                    ...
                    if($this->cooperator_model->check_cooperator_in_cooperative_laboratories($decoded_id,$decoded_cooperator_id)){ //check if cooperator is in cooperative
                        ...
                        if($temp == FALSE){
                          ...
                          $this->load->view('./template/header', $data);
                          $this->load->view('update/laboratories/edit_form_cooperator_laboratory', $data);
                          $this->load->view('./template/footer');
                        }else{
                          ...
                          if($this->edit_no_changes(strtolower($items['full_name']),strtolower($items['middle_name']),strtolower($items['last_name']),$items['id']))
                          {
	                          ...
	                          if($success['success']){
	                            $this->session->set_flashdata('member_success', $success['message']);
	                            redirect('laboratories_update/'.$coopID.'/laboratories_cooperators_update');
	                          }else{
	                            $this->session->set_flashdata('member_error', $success['message']);
	                            redirect('laboratories_update/'.$coopID.'/laboratories_cooperators_update');
	                          }
  ...
  function delete_cooperator(){
    if(!$this->session->userdata('logged_in')){
      ...
      if($this->input->post('deleteCooperatorBtn')){
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            if($this->laboratories_model->check_own_branch_updating($decoded_id,$user_id)){
              $decoded_post_cooperator_id = $this->encryption->decrypt(decrypt_custom($this->input->post('cooperatorID')));
              if($this->cooperator_model->check_cooperator_in_cooperative_laboratories($decoded_id,$decoded_post_cooperator_id)){
                // if(!$this->laboratories_model->check_submitted_for_evaluation($decoded_id)){
                  ...
                  if($success){
                    ...
                    redirect('laboratories_update/'.$this->input->post('cooperativeID').'/laboratories_cooperators_update');
                  }else{
