...
class Committees_update extends CI_Controller{
                      ...
                      // if($data['article_complete'] || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                        // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                          if(!isset($_POST['addCommitteeBtn'])){
                            ...
                            $data['cooperators'] = $this->cooperators_update_model->get_all_cooperator_of_coop_for_committee($decoded_id,$user_id);
                            ...
                            if($data['coop_info']->grouping == 'Federation'){
                                $data['cooperators_federation'] = $this->cooperators_update_model->get_all_cooperator_of_coop_for_committee_federation($result_array,$user_id);
                            } else if($data['coop_info']->grouping == 'Union' && $data['coop_info']->type_of_cooperative == 'Union'){
                                $data['cooperators_union'] = $this->cooperators_update_model->get_all_cooperator_of_coop_for_committee_union($result_array_union,$user_id);
                            }
                            $this->load->view('./template/header', $data);

                            if($data['coop_info']->grouping == 'Federation'){
                                $this->load->view('update/committees/add_form_committee_federation', $data);
                            } else if($data['coop_info']->grouping == 'Union' && $data['coop_info']->type_of_cooperative == 'Union'){
                                $this->load->view('update/committees/add_form_committee_union', $data);
                            } else {
                                $this->load->view('update/committees/add_form_committee', $data);
                            }
                            ...
                            if($data['coop_info']->grouping == 'Federation'){
                                ...
                                if ($this->committee_model->isExisting($decoded_id)){
                                  ...
                                  $success = $this->committee_update_model->add_committee($data);
                                  if($success['success']){
                                    $this->session->set_flashdata('committee_success', $success['message']);
                                    redirect('cooperatives_update/'.$this->input->post('cooperativesID').'/committees_update');
                                  }else{
                                    $this->session->set_flashdata('committee_error', $success['message']);
                                    redirect('cooperatives_update/'.$this->input->post('cooperativesID').'/committees_update');
                                  }
                          ...
                          // if($data['article_complete'] || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                            if(!isset($_POST['addCommitteeBtn'])){
                              ...
                              if(!$data['is_client']){
                                ...
                                $user_id = $data['coop_info']->users_id;
                        ...
                        if($this->committee_model->$committee_edit($decoded_committee_id,$decoded_id)){ //check if committee is in cooperative
                          // if(!$this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                            if($this->form_validation->run() == FALSE){
                              ...
                              $data['cooperators'] = $this->cooperator_model->get_all_cooperator_of_coop_for_committee($decoded_id,$user_id);
                              $this->load->view('./template/header', $data);
                              $this->load->view('update/committees/edit_form_committee', $data);
                              $this->load->view('./template/footer');
                            }else{
                              $decoded_id = $this->encryption->decrypt(decrypt_custom($this->input->post('cooperatorID')));
