...
use Dompdf\Options;
class Documents_transfer extends CI_Controller{
   var $pageCount = array();
...
public function list_upload_pdf_branch($id =null,$doc_type=null)
{
  ...
  else
  {
    ...
    if($data['is_client'] ==1)
    {
      ...
      $this->load->view('template/header',$data);
      $this->load->view('documents/list_of_uploaded_pdf_branch_transfer',$data);
...
//                if($this->branches_model->check_submitted_for_evaluation($decoded_id)) {
                  ...
                  // $data['last_query'] = $this->db->last_query();
                  // New
                  $data['branches_comments_cds'] = $this->branches_model->branches_comments_cds_transfer($branch_info->id);
                  $data['branches_comments_snr'] = $this->branches_model->branches_transfer_comments_snr($branch_info->id,$evaluator_number);
                  $data['branches_comments_snr_limit'] = $this->branches_model->branches_comments_snr_transfer_limit($branch_info->id,$evaluator_number);
                  //
                ...
                $branch_trans = $this->branches_model->get_branch_to_be_transferred($branch_info->user_id,$branch_info->id);
                $data['branch_trans'] = $branch_trans;
                // echo $this->db->last_query();
                ...
                if($data['branch_info']->area_of_operation == 'Interregional'){
                  ...
                  $this->load->view('templates/admin_header', $data);
                  $this->load->view('documents/list_of_documents_branch_transfer', $data);
                  $this->load->view('cooperative/evaluation/approve_modal_branch_for_transfer');
                  $this->load->view('cooperative/evaluation/deny_modal_branch_for_transfer');
                  $this->load->view('cooperative/evaluation/defer_modal_branch_for_transfer');
                  $this->load->view('templates/admin_footer');
//                } else{
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_letter_of_intent_trans', $data);
                $this->load->view('./template/footer');
          }else{
            echo $decoded_id.'-'.$user_id;
            // $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!!!!.');
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            if(!($this->upload->do_upload('file43'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_transfer');
            }else{
                if($this->input->post('status')==17){
                    $status = 2;
                } else {
