...
class Capitalization_update extends CI_Controller{
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            ...
            if(!$this->cooperatives_update_model->check_expired_reservation($decoded_id)){
                $data['coop_info'] = $this->cooperatives_update_model->get_cooperative_info($user_id,$decoded_id);
...
//                    $data['requirements_complete'] = $this->capitalization_update_model->is_requirements_complete($decoded_id);
                    ...
                    if($this->input->post('capitalizationPrimaryBtn')) {
                      ...
                      $data = $this->input->post('item');
                       if($this->capitalization_update_model->update_capitalization($decoded_post_coop_id,$data)){
                            $this->session->set_flashdata('capitalization_success', 'Successfully Updated');
                            redirect('cooperatives_update/'.$this->input->post('cooperativesID').'/capitalization_update');
                        }else{
                    ...
                    $this->load->view('update/capitalization_form_update', $data);
                ...
                redirect('cooperatives_update/'.$id.'capitalization_update');
              }
            }else{
