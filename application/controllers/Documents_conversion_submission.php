...
use Dompdf\Options;
class Documents_conversion_submission extends CI_Controller{
   var $pageCount = array();
...
public function list_upload_pdf_branch($id =null,$doc_type=null)
{
  ...
  else
  {
    ...
    if($data['is_client'] ==1)
    {
      ...
      $this->load->view('template/header',$data);
      $this->load->view('documents/list_of_uploaded_pdf_branch_closure',$data);
        ...
        if(is_numeric($decoded_id) && $decoded_id!=0){
          if($this->session->userdata('client')){
            if($this->branches_model->check_own_branch($decoded_id,$user_id)){
                ...
                $data['document_50'] = $this->uploaded_document_model->get_document_50_info($branch_info->id,$branch_info->application_id);
                $data['document_51'] = $this->uploaded_document_model->get_document_51_info($branch_info->id,$branch_info->application_id);
                $data['document_52'] = $this->uploaded_document_model->get_document_52_info($branch_info->id,$branch_info->application_id);
                $data['document_53'] = $this->uploaded_document_model->get_document_53_info($branch_info->id,$branch_info->application_id);
                $data['document_54'] = $this->uploaded_document_model->get_document_54_info($branch_info->id,$branch_info->application_id);

                $data['document_others_unifed'] = $this->uploaded_document_model->get_document_42_info($branch_info->id,$branch_info->application_id);
                $data['document_letter_of_intent'] = $this->uploaded_document_model->get_document_43_info($branch_info->id,$branch_info->application_id);
                ...
                $this->load->view('template/header', $data);
                $this->load->view('documents/list_of_documents_branch_conversion', $data);
                $this->load->view('template/footer');
...
//                if($this->branches_model->check_submitted_for_evaluation($decoded_id)) {
                ...
                $data['document_9'] = $this->uploaded_document_model->get_document_9_info($branch_info->id,$branch_info->application_id);
                $data['document_40'] = $this->uploaded_document_model->get_document_40_info($branch_info->id,$branch_info->application_id);
                $data['document_44'] = $this->uploaded_document_model->get_document_44_info($branch_info->id,$branch_info->application_id);
                $data['document_45'] = $this->uploaded_document_model->get_document_45_info($branch_info->id,$branch_info->application_id);
                $data['document_46'] = $this->uploaded_document_model->get_document_46_info($branch_info->id,$branch_info->application_id);
                $data['document_others_unifed'] = $this->uploaded_document_model->get_document_42_info($branch_info->id,$branch_info->application_id);
                $data['document_letter_of_intent'] = $this->uploaded_document_model->get_document_43_info($branch_info->id,$branch_info->application_id);
                ...
                if($data['branch_info']->area_of_operation == 'Interregional'){
                  ...
                  $this->load->view('templates/admin_header', $data);
                  $this->load->view('documents/list_of_documents_branch_conversion', $data);
                  $this->load->view('cooperative/evaluation/approve_modal_branch_for_closure');
            ...
            if($this->cooperatives_model->check_expired_reservation_by_admin($decoded_id)){
              ...
              if($this->cooperatives_model->check_submitted_for_evaluation($decoded_id)){
                ...
                if($data['bylaw_complete']){
                    ...
                    $capitalization_info = $data['capitalization_info'];
                    $data['cooperator_complete'] = $this->cooperator_model->is_requirements_complete($decoded_id,$data['capitalization_info']->associate_members);
                    if($data['cooperator_complete'] || $data['coop_info']->grouping == 'Federation' || ($data['coop_info']->grouping == 'Union' && $data['coop_info']->type_of_cooperative == 'Union')){
                      $data['purposes_complete'] = $this->purpose_model->check_purpose_complete($decoded_id);
                      if($data['purposes_complete']){
                        $data['article_complete'] = ($data['coop_info']->category_of_cooperative=="Primary") ? $this->article_of_cooperation_model->check_article_primary_complete($decoded_id) : true;
                        if($data['article_complete'] || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                    ...
                    if($data['bylaw_complete']){
                        ...
                        // if($data['cooperator_complete'] || ($data['coop_info']->grouping == 'Union' && $data['coop_info']->type_of_cooperative == 'Union') || $data['coop_info']->grouping == 'Federation'){
                          ...
                          if($data['purposes_complete']){
                            ...
                            if($data['article_complete'] || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                              $data['committees_complete'] = $this->committee_model->committee_complete_count($decoded_id);
                              if($data['committees_complete'] || ($data['coop_info']->grouping == 'Union' && $data['coop_info']->type_of_cooperative == 'Union') || $data['coop_info']->grouping == 'Federation'){
                                $data['economic_survey_complete'] = $this->economic_survey_model->check_survey_complete($decoded_id);
                                if($data['economic_survey_complete'] || $data['coop_info']->category_of_cooperative = 'Secondary' || $data['coop_info']->category_of_cooperative = 'Tertiary'){
                                  $data['staff_complete'] = $this->staff_model->requirements_complete($decoded_id);
                                  if($data['staff_complete']){
...
//          }
  ...
  function upload_document_for_conversion_50($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_conversion_50', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_conversion_51($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_conversion_51', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_conversion_52($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_conversion_52', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_conversion_53($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_conversion_53', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_for_conversion_54($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_for_conversion_54', $data);
                $this->load->view('./template/footer');
          }else{
            $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!.');
            redirect('branches');
  ...
  function upload_document_letter_of_intent($id = null){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      $decoded_id = $this->encryption->decrypt(decrypt_custom($id));
        ...
        if($this->session->userdata('client')){
          if($this->branches_model->check_own_branch($decoded_id,$user_id)){
              $data['coop_info'] = $this->branches_model->get_branch_info($user_id,$decoded_id);
                ...
                $data['coopid'] = $decoded_id;
                $this->load->view('./template/header', $data);
                $this->load->view('cooperative/upload_form/upload_document_letter_of_intent', $data);
                $this->load->view('./template/footer');
          }else{
            echo $decoded_id.'-'.$user_id;
            // $this->session->set_flashdata('redirect_applications_message', 'Unauthorized!!!!!.');
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            $config['upload_path'] = UPLOAD_DIR;
            $config['file_name'] = $random_.'_'.$decoded_uid.'_'.$decoded_id.'_nt.pdf';
            $config['allowed_types'] = 'pdf';
            $config['overwrite'] = true;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file53'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
            }else{
                if($this->input->post('status')==17){
                    $status = 2;
                } else {
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,53,$this->upload->data('file_name'),$status)){
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file54'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,54,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            ...
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file52'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,52,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            $config['upload_path'] = UPLOAD_DIR;
            $config['file_name'] = $random_.'_'.$decoded_uid.'_'.$decoded_id.'_man_chair.pdf';
            $config['allowed_types'] = 'pdf';
            $config['overwrite'] = true;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file51'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,51,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            $config['upload_path'] = UPLOAD_DIR;
            $config['file_name'] = $random_.'_'.$decoded_uid.'_'.$decoded_id.'_loa.pdf';
            $config['allowed_types'] = 'pdf';
            $config['overwrite'] = true;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file50'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
            }else{
                if($this->input->post('status')==17){
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,50,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_conversion_submission');
              }else{
                $file = $config['upload_path'].$config['file_name'];
...
//          }
  ...
  function do_upload_letter_of_intent(){
    if(!$this->session->userdata('logged_in')){
      redirect('users/login');
    }else{
      if($this->input->post('uploadOtherDocumentBtn')){
...
//          if(!$this->branches_model->check_submitted_for_evaluation($decoded_id)){
           $random_ = random_string('alnum',5);
            $config['upload_path'] = UPLOAD_DIR;
            $config['file_name'] = $this->input->post('file43');
            $config['allowed_types'] = 'pdf';
            $config['overwrite'] = true;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            if(!($this->upload->do_upload('file43'))){
              $this->session->set_flashdata('document_40_error', $this->upload->display_errors('<p>', '</p>'));
              redirect('branches/'.$this->input->post('branchID').'/documents_closure');
            }else{
                if($this->input->post('status')==17){
                    $status = 2;
                } else {
              ...
              $application_id = $this->input->post('application_id');
              $data = array('upload_data' => $this->upload->data());
              if($this->uploaded_document_model->add_document_info_($decoded_id,$application_id,43,$this->upload->data('file_name'),$status)){
                $this->session->set_flashdata('document_40_success', 'Successfully uploaded.');
                redirect('branches/'.$this->input->post('branchID').'/documents_closure');
              }else{
                $file = $config['upload_path'].$config['file_name'];
        ...
        if($this->session->userdata('client')){
          ...
          if($this->session->userdata('access_level')==5){
            ...
            if($this->cooperatives_model->check_expired_reservation_by_admin($decoded_id)){
              ...
              $this->load->library('session');
              $path = 'articles_of_cooperation_for_primary_branch.pdf';
              $getTotalPages = $f->get_canvas()->get_page_count();
              $user_data = array(
                // 'pagecount' => $canvas->page_text(5, 5, "{PAGE_COUNT}", '', 8, 0)
                'pagecount' => $getTotalPages
