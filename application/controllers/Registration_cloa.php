...
class registration_cloa extends CI_Controller{
  ...
  function branch($id = null) {
    if(!$this->session->userdata('logged_in')){
      ...
      if ($branch_info->status==20)
        ...
        $branch_details = $this->branches_model->get_branch_info_by_admin_transfer($decoded_id);
        ...
        $html2 = $this->load->view('cooperative/CAT_view', $data1, TRUE);
        // $J = new pdf();       
        // $J->set_option('isRemoteEnabled',TRUE);
        // $J->setPaper('folio', 'portrait');
        // $J->load_html($html2);
