<?php
  ...
  class Denied_defered_cooperatives extends CI_Controller{
...
//          $data['list_cooperatives'] = $this->cooperatives_model->get_all_cooperatives($this->session->userdata('user_id'));
          ...
          if($this->session->userdata('access_level')==5){
            ...
            $data['title'] = 'List of Cooperatives';
            $data['header'] = 'Deferred/Denied';
            $data['admin_info'] = $this->admin_model->get_admin_info($user_id);
            
            $config["base_url"] = base_url() . "denied_defered_cooperatives";
            $config["total_rows"] = $this->cooperatives_model->get_all_cooperatives_by_senior_defer_deny_count($data['admin_info']->region_code);
            $config["per_page"] = 5;
            ...
            if($this->session->userdata('access_level')==1){
              ...
              if($data['admin_info']->region_code=="00"){
                ...
                $data['list_cooperatives_defer_deny'] = $this->cooperatives_model->get_all_cooperatives_by_senior_defer_deny2($data['admin_info']->region_code, $config["per_page"], $per_page);
                $data['list_specialist'] = $this->admin_model->get_all_specialist_by_region($data['admin_info']->region_code);
              }
              // echo '<pre>';
            ...
            $this->load->view('applications/list_of_applications_dd', $data);
